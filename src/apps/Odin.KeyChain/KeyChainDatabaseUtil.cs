...
public static class KeyChainDatabaseUtil
{
    ...
    public static async Task InitializeDatabaseAsync(KeyChainDatabase db)
    {
        ...
        if (r == null)
        {
            ...
            await db.KeyChain.InsertAsync(genesis);
        }
    }
    ...
    public static async Task<bool> VerifyEntireBlockChainAsync(KeyChainDatabase db)
    {
        await using var cn = await db.CreateScopedConnectionAsync();
        await using var cmd = cn.CreateCommand();

        cmd.CommandText = "SELECT rowid,previousHash,identity,timestamp,signedPreviousHash,algorithm,publicKeyJwkBase64Url,recordHash FROM keyChain ORDER BY rowid ASC;";
        ...
        {
            var record = db.KeyChain.ReadRecordFromReaderAll(rdr);
