...
public static class DevEnvironmentSetup
{
        ...
        IIdentityRegistry identityRegistry)
    {
        ...
        if (odinConfiguration.Development?.PreconfiguredDomains.Any() ?? false)
        {
            ...
            foreach (var domain in odinConfiguration.Development.PreconfiguredDomains)
            {
                ...
                certificates.Add(CreateGuidForSillyHuman(++idx), domain);
            }
        }
    ...
    private static Guid CreateGuidForSillyHuman(ulong number)
    {
        // Note: no overflow check, just don't do that many.
        var firstPart = ((number << 28) & 0xFFFFFFFFUL).ToString("X8");
        var lastPart = number.ToString("X12");
        var guidStr = $"{firstPart}-0000-0000-0000-{lastPart}";
