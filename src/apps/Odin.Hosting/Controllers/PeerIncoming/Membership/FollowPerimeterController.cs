...
namespace Odin.Hosting.Controllers.PeerIncoming.Membership
{
    ...
    public class FollowPerimeterController : OdinControllerBase
    {
        ...
        public async Task<IActionResult> ReceiveFollowRequest([FromBody] RsaEncryptedPayload payload)
        {
            ...
            var (isValidPublicKey, payloadBytes) = await _publicPrivatePublicKeyService.RsaDecryptPayload(RsaKeyType.OfflineKey, payload, TheOdinContext);
            ...
            await _followerPerimeterService.AcceptFollower(request, TheOdinContext);
        ...
        {
            await _followerPerimeterService.AcceptUnfollowRequest(TheOdinContext);
            return Ok();
        }
    }
}