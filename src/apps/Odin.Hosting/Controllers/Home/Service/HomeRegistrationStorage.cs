...
{
    public class HomeRegistrationStorage(TableKeyValueCached tblKeyValue)
    {
        ...
        {
            var client = await ClientStorage.GetAsync<HomeAppClient>(tblKeyValue, id);
            return client;
        }
        ...
        public async Task SaveClientAsync(HomeAppClient client)
        {
            ...
            await ClientStorage.UpsertAsync(tblKeyValue, client.AccessRegistration.Id, client);
        }

        public async Task DeleteClientAsync(GuidId accessRegistrationId)
        {
            
            await ClientStorage.DeleteAsync(tblKeyValue, accessRegistrationId);
        }
    }
}