...
namespace Odin.Hosting.Controllers.OwnerToken.AppManagement
{
        ...
        : OdinControllerBase
    {
        ...
        //
        //     var (reg, _) = await appRegistrationService.RegisterClientPkAsync(request.AppId, clientPublicKey, request.ClientFriendlyName,
        //         WebOdinContext);
        //     return reg;
        // }

        [HttpPost("register/client-ecc")]
        public async Task<AppClientEccRegistrationResponse> RegisterClientUsingEcc([FromBody] AppClientEccRegistrationRequest request)
        {
            OdinValidationUtils.AssertNotNull(request, nameof(request));
            OdinValidationUtils.AssertIsTrue(request.AppId != Guid.Empty, "missing app id");
            OdinValidationUtils.AssertNotNullOrEmpty(request.ClientFriendlyName, nameof(request.ClientFriendlyName));
