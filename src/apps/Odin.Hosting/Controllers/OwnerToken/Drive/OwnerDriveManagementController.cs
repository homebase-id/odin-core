...
namespace Odin.Hosting.Controllers.OwnerToken.Drive
{
    ...
    public class OwnerDriveManagementController : OdinControllerBase
    {
        ...
        {
            var drives = await _driveManager.GetDrives(new PageOptions(request.PageNumber, request.PageSize), TheOdinContext);
        ...
        public async Task<bool> CreateDrive([FromBody] CreateDriveRequest request)
        {
            ...
            var _ = await _driveManager.CreateDrive(request, TheOdinContext);
            return true;
        }
        ...
        public async Task<bool> UpdateDriveMetadata([FromBody] UpdateDriveDefinitionRequest request)
        {
            ...
            await _driveManager.UpdateMetadata(driveId.GetValueOrDefault(), request.Metadata, TheOdinContext);
            return true;
        }
        ...
        {
            var driveId = await _driveManager.GetDriveIdByAlias(request.TargetDrive, true);
            await _driveManager.SetDriveReadMode(driveId.GetValueOrDefault(), request.AllowAnonymousReads, TheOdinContext);
            return Ok();
        }
