...
#pragma warning restore CS0168
        ...
        protected virtual async Task<(List<NonceRecord>, Int64? nextCursor)> PagingByRowIdAsync(int count, Guid identityId, Int64? inCursor)
        {
            if (count < 1)
                throw new Exception("Count must be at least 1.");
            if (count == int.MaxValue)
            ...
            {
                getPaging0Command.CommandText = "SELECT rowId,identityId,id,expiration,data,created,modified FROM Nonce " +
