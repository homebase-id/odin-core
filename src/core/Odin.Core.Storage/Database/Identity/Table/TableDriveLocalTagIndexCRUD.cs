...
#pragma warning restore CS0168
        ...
        protected DriveLocalTagIndexRecord ReadRecordFromReader2(DbDataReader rdr)
        {
            var result = new List<DriveLocalTagIndexRecord>();
#pragma warning disable CS0168
            long bytesRead;
#pragma warning restore CS0168
        ...
        protected virtual async Task<List<DriveLocalTagIndexRecord>> GetAllAsync()
        {
            await using var cn = await _scopedConnectionFactory.CreateScopedConnectionAsync();
            await using var get2Command = cn.CreateCommand();
            {
                get2Command.CommandText = "SELECT identityId,driveId,fileId,tagId FROM driveLocalTagIndex " +
                                             ";";

                {
                    using (var rdr = await get2Command.ExecuteReaderAsync(CommandBehavior.Default))
                    {
                        if (await rdr.ReadAsync() == false)
                        {
                            return new List<DriveLocalTagIndexRecord>();
                        }
                        var result = new List<DriveLocalTagIndexRecord>();
                        while (true)
                        {
