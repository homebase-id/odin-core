...
namespace Odin.Core.Storage.Database.Identity.Table
{
    ...
    public abstract class TableDriveLocalTagIndexCRUD : TableBase
    {
        ...
        *
        public override async Task EnsureTableExistsAsync(bool dropExisting = false)
        {
            ...
            if (dropExisting)
                await SqlHelper.DeleteTableAsync(cn, "DriveLocalTagIndex");
            var rowid = "";
            var commentSql = "";
            if (cn.DatabaseType == DatabaseType.Postgres)
            {
               rowid = "rowId BIGSERIAL PRIMARY KEY,";
               commentSql = "COMMENT ON TABLE DriveLocalTagIndex IS '{ \"Version\": 0 }';";
            }
            else
               rowid = "rowId INTEGER PRIMARY KEY AUTOINCREMENT,";
            var wori = "";
            string createSql =
                "CREATE TABLE IF NOT EXISTS DriveLocalTagIndex( -- { \"Version\": 0 }\n"
                   +rowid
                   ...
                   +$"){wori};"
                   ;
            await SqlHelper.CreateTableWithCommentAsync(cn, "DriveLocalTagIndex", createSql, commentSql);
        }
       */
        ...
        protected virtual async Task<int> InsertAsync(DriveLocalTagIndexRecord item)
        {
            ...
            await using var insertCommand = cn.CreateCommand();
            {
                ...
                insertCommand.AddParameter("@tagId", DbType.Binary, item.tagId);
        ...
        protected virtual async Task<int> UpsertAsync(DriveLocalTagIndexRecord item)
        {
            ...
            await using var upsertCommand = cn.CreateCommand();
            {
                ...
                upsertCommand.AddParameter("@tagId", DbType.Binary, item.tagId);
        ...
        protected virtual async Task<int> UpdateAsync(DriveLocalTagIndexRecord item)
        {
            ...
            await using var updateCommand = cn.CreateCommand();
            {
                ...
                updateCommand.AddParameter("@tagId", DbType.Binary, item.tagId);
...
#pragma warning restore CS0168
        ...
        internal virtual async Task ExportRowsAsync(Guid identityId, Func<DriveLocalTagIndexRecord, Task> onRow)
        {
            await using var cn = await _scopedConnectionFactory.CreateScopedConnectionAsync();
            await using var exportCommand = cn.CreateCommand();
            {
                exportCommand.CommandText = "SELECT rowId,identityId,driveId,fileId,tagId FROM DriveLocalTagIndex " +
        ...
        internal virtual async Task<int> ImportRowAsync(DriveLocalTagIndexRecord item)
        {
            await using var cn = await _scopedConnectionFactory.CreateScopedConnectionAsync();
            await using var importCommand = cn.CreateCommand();
            {
                importCommand.CommandText = "INSERT INTO DriveLocalTagIndex (identityId,driveId,fileId,tagId) " +
                                            "VALUES (@identityId,@driveId,@fileId,@tagId);";
                ...
                importCommand.AddParameter("@tagId", DbType.Binary, item.tagId);
        ...
        protected virtual async Task<int> DeleteAsync(Guid identityId,Guid driveId,Guid fileId,Guid tagId)
        {
            ...
            await using var delete1Command = cn.CreateCommand();
            {
                ...
                delete1Command.AddParameter("@tagId", DbType.Binary, tagId);
                var count = await delete1Command.ExecuteNonQueryAsync();
                return count;
            }
        }

        protected virtual async Task<DriveLocalTagIndexRecord> PopAsync(Guid identityId,Guid driveId,Guid fileId,Guid tagId)
        {
            await using var cn = await _scopedConnectionFactory.CreateScopedConnectionAsync();
            await using var deleteCommand = cn.CreateCommand();
            {
                deleteCommand.CommandText = "DELETE FROM DriveLocalTagIndex " +
                                             "WHERE identityId = @identityId AND driveId = @driveId AND fileId = @fileId AND tagId = @tagId " + 
                                             "RETURNING rowId";
                ...
                deleteCommand.AddParameter("@tagId", DbType.Binary, tagId);
...
#pragma warning restore CS0168
        ...
        protected virtual async Task<DriveLocalTagIndexRecord> GetAsync(Guid identityId,Guid driveId,Guid fileId,Guid tagId)
        {
            ...
            await using var get0Command = cn.CreateCommand();
            {
                ...
                get0Command.AddParameter("@tagId", DbType.Binary, tagId);
                {
        ...
        protected virtual async Task<(List<DriveLocalTagIndexRecord>, Int64? nextCursor)> PagingByRowIdAsync(int count, Guid identityId, Int64? inCursor)
        {
            if (count < 1)
