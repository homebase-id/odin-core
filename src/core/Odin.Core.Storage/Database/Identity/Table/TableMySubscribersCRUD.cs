...
{
    public record MySubscribersRecord
    {
    ...
    public abstract class TableMySubscribersCRUD : TableBase
    {
        private readonly ScopedIdentityConnectionFactory _scopedConnectionFactory;
        public override string TableName { get; } = "MySubscribers";

        protected TableMySubscribersCRUD(ScopedIdentityConnectionFactory scopedConnectionFactory)
        {
        ...
        *
        public override async Task EnsureTableExistsAsync(bool dropExisting = false)
        {
            ...
            if (dropExisting)
                await SqlHelper.DeleteTableAsync(cn, "MySubscribers");
            var rowid = "";
            var commentSql = "";
            if (cn.DatabaseType == DatabaseType.Postgres)
            {
               rowid = "rowId BIGSERIAL PRIMARY KEY,";
               commentSql = "COMMENT ON TABLE MySubscribers IS '{ \"Version\": 0 }';";
            }
            else
               rowid = "rowId INTEGER PRIMARY KEY AUTOINCREMENT,";
            var wori = "";
            string createSql =
                "CREATE TABLE IF NOT EXISTS MySubscribers( -- { \"Version\": 0 }\n"
                   +rowid
                   ...
                   ;
            await SqlHelper.CreateTableWithCommentAsync(cn, "MySubscribers", createSql, commentSql);
        }
       */

        protected virtual async Task<int> InsertAsync(MySubscribersRecord item)
        {
            item.Validate();
            await using var cn = await _scopedConnectionFactory.CreateScopedConnectionAsync();
        ...
        protected virtual async Task<bool> TryInsertAsync(MySubscribersRecord item)
        {
            item.Validate();
            await using var cn = await _scopedConnectionFactory.CreateScopedConnectionAsync();
            await using var insertCommand = cn.CreateCommand();
        ...
        protected virtual async Task<int> UpsertAsync(MySubscribersRecord item)
        {
            item.Validate();
            await using var cn = await _scopedConnectionFactory.CreateScopedConnectionAsync();
        ...
        protected virtual async Task<int> UpdateAsync(MySubscribersRecord item)
        {
            item.Validate();
            await using var cn = await _scopedConnectionFactory.CreateScopedConnectionAsync();
        ...
        protected new async Task<int> GetCountAsync()
        {
            ...
            await using var getCountCommand = cn.CreateCommand();
            {
                ...
                getCountCommand.CommandText = "SELECT COUNT(*) FROM MySubscribers;";
...
#pragma warning restore CS0168
            ...
            var item = new MySubscribersRecord();
        ...
        protected virtual async Task<int> DeleteBySubscriberOdinIdAsync(Guid identityId,OdinId subscriberOdinId)
        {
            ...
            {
                delete0Command.CommandText = "DELETE FROM MySubscribers " +
...
#pragma warning restore CS0168
        ...
        protected MySubscribersRecord ReadRecordFromReader1(DbDataReader rdr,Guid identityId)
        {
            var result = new List<MySubscribersRecord>();
#pragma warning disable CS0168
            long bytesRead;
#pragma warning restore CS0168
        ...
        protected virtual async Task<List<MySubscribersRecord>> GetAllAsync(Guid identityId)
        {
            await using var cn = await _scopedConnectionFactory.CreateScopedConnectionAsync();
            await using var get1Command = cn.CreateCommand();
            {
                ...
                get1Command.AddParameter("@identityId", DbType.Binary, identityId);
                {
                    using (var rdr = await get1Command.ExecuteReaderAsync(CommandBehavior.Default))
                    {
                        if (await rdr.ReadAsync() == false)
                        {
                            return new List<MySubscribersRecord>();
                        }
                        var result = new List<MySubscribersRecord>();
                        while (true)
                        {
