...
#pragma warning restore CS0168
        ...
        protected AppNotificationsRecord ReadRecordFromReader0(DbDataReader rdr)
        {
            var result = new List<AppNotificationsRecord>();
#pragma warning disable CS0168
            long bytesRead;
#pragma warning restore CS0168
        ...
        protected virtual async Task<List<AppNotificationsRecord>> GetAllAsync()
        {
            await using var cn = await _scopedConnectionFactory.CreateScopedConnectionAsync();
            await using var get0Command = cn.CreateCommand();
            {
                get0Command.CommandText = "SELECT rowId,identityId,notificationId,unread,senderId,timestamp,data,created,modified FROM AppNotifications " +
                                             "ORDER BY rowId ASC "+
                                             ";";

                {
                    using (var rdr = await get0Command.ExecuteReaderAsync(CommandBehavior.Default))
                    {
                        ...
                        {
                            return new List<AppNotificationsRecord>();
                        }
                        var result = new List<AppNotificationsRecord>();
                        while (true)
                        {
        ...
        protected AppNotificationsRecord ReadRecordFromReader1(DbDataReader rdr,Guid identityId,Guid notificationId)
        {
            var result = new List<AppNotificationsRecord>();
#pragma warning disable CS0168
            long bytesRead;
#pragma warning restore CS0168
        ...
        protected virtual async Task<AppNotificationsRecord> GetAsync(Guid identityId,Guid notificationId)
        {
            ...
            {
                get1Command.CommandText = "SELECT rowId,unread,senderId,timestamp,data,created,modified FROM AppNotifications " +
