...
#pragma warning restore CS0168
        ...
        internal virtual async Task ExportRowsAsync(Guid identityId, Func<ImFollowingRecord, Task> onRow)
        {
            await using var cn = await _scopedConnectionFactory.CreateScopedConnectionAsync();
            await using var exportCommand = cn.CreateCommand();
            {
                exportCommand.CommandText = "SELECT rowId,identityId,identity,driveId,created,modified FROM ImFollowing " +
        ...
        internal virtual async Task<int> ImportRowAsync(ImFollowingRecord item)
        {
            await using var cn = await _scopedConnectionFactory.CreateScopedConnectionAsync();
            await using var importCommand = cn.CreateCommand();
            {
                importCommand.CommandText = "INSERT INTO ImFollowing (identityId,identity,driveId,created,modified) " +
...
#pragma warning restore CS0168
        ...
        protected virtual async Task<(List<ImFollowingRecord>, Int64? nextCursor)> PagingByRowIdAsync(int count, Guid identityId, Int64? inCursor)
        {
            if (count < 1)
