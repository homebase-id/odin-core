...
#pragma warning restore CS0168
        ...
        internal virtual async Task ExportRowsAsync(Guid identityId, Func<KeyTwoValueRecord, Task> onRow)
        {
            await using var cn = await _scopedConnectionFactory.CreateScopedConnectionAsync();
            await using var exportCommand = cn.CreateCommand();
            {
                exportCommand.CommandText = "SELECT rowId,identityId,key1,key2,data FROM KeyTwoValue " +
        ...
        internal virtual async Task<int> ImportRowAsync(KeyTwoValueRecord item)
        {
            await using var cn = await _scopedConnectionFactory.CreateScopedConnectionAsync();
            await using var importCommand = cn.CreateCommand();
            {
                importCommand.CommandText = "INSERT INTO KeyTwoValue (identityId,key1,key2,data) " +
                                            "VALUES (@identityId,@key1,@key2,@data);";
...
#pragma warning restore CS0168
        ...
        protected virtual async Task<(List<KeyTwoValueRecord>, Int64? nextCursor)> PagingByRowIdAsync(int count, Guid identityId, Int64? inCursor)
        {
            if (count < 1)
