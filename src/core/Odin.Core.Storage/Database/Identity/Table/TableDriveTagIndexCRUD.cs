...
namespace Odin.Core.Storage.Database.Identity.Table
{
    ...
    public abstract class TableDriveTagIndexCRUD : TableBase
    {
        ...
        public override string TableName { get; } = "DriveTagIndex";

        protected TableDriveTagIndexCRUD(ScopedIdentityConnectionFactory scopedConnectionFactory)
        {
        ...
        *
        public override async Task EnsureTableExistsAsync(bool dropExisting = false)
        {
            ...
            if (dropExisting)
                await SqlHelper.DeleteTableAsync(cn, "DriveTagIndex");
            var rowid = "";
            var commentSql = "";
            ...
            string createSql =
                "CREATE TABLE IF NOT EXISTS DriveTagIndex( -- { \"Version\": 0 }\n"
                   ...
                   +$"){wori};"
                   ;
            await SqlHelper.CreateTableWithCommentAsync(cn, "DriveTagIndex", createSql, commentSql);
        }
       */
...
#pragma warning restore CS0168
        ...
        internal virtual async Task ExportRowsAsync(Guid identityId, Func<DriveTagIndexRecord, Task> onRow)
        {
            await using var cn = await _scopedConnectionFactory.CreateScopedConnectionAsync();
            await using var exportCommand = cn.CreateCommand();
            {
                exportCommand.CommandText = "SELECT rowId,identityId,driveId,fileId,tagId FROM DriveTagIndex " +
        ...
        internal virtual async Task<int> ImportRowAsync(DriveTagIndexRecord item)
        {
            await using var cn = await _scopedConnectionFactory.CreateScopedConnectionAsync();
            await using var importCommand = cn.CreateCommand();
            {
                importCommand.CommandText = "INSERT INTO DriveTagIndex (identityId,driveId,fileId,tagId) " +
                                            "VALUES (@identityId,@driveId,@fileId,@tagId);";
...
#pragma warning restore CS0168
        ...
        protected virtual async Task<(List<DriveTagIndexRecord>, Int64? nextCursor)> PagingByRowIdAsync(int count, Guid identityId, Int64? inCursor)
        {
            if (count < 1)
