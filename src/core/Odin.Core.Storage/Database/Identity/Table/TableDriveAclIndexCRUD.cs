...
#pragma warning restore CS0168
        ...
        protected DriveAclIndexRecord ReadRecordFromReader2(DbDataReader rdr)
        {
            var result = new List<DriveAclIndexRecord>();
#pragma warning disable CS0168
            long bytesRead;
#pragma warning restore CS0168
        ...
        protected virtual async Task<List<DriveAclIndexRecord>> GetAllAsync()
        {
            await using var cn = await _scopedConnectionFactory.CreateScopedConnectionAsync();
            await using var get2Command = cn.CreateCommand();
            {
                get2Command.CommandText = "SELECT identityId,driveId,fileId,aclMemberId FROM driveAclIndex " +
                                             ";";

                {
                    using (var rdr = await get2Command.ExecuteReaderAsync(CommandBehavior.Default))
                    {
                        if (await rdr.ReadAsync() == false)
                        {
                            return new List<DriveAclIndexRecord>();
                        }
                        var result = new List<DriveAclIndexRecord>();
                        while (true)
                        {
