...
namespace Odin.Core.Storage.Database.Identity.Table
{
    ...
    public abstract class TableDriveTransferHistoryCRUD
    {
        ...
        protected virtual async Task<int> InsertAsync(DriveTransferHistoryRecord item)
        {
            ...
            {
                insertCommand.CommandText = "INSERT INTO driveTransferHistory (identityId,driveId,fileId,remoteIdentityId,status) " +
                                             "VALUES (@identityId,@driveId,@fileId,@remoteIdentityId,@status)";
                ...
                insertParam5.ParameterName = "@status";
                ...
                insertParam5.Value = item.status;
        ...
        protected virtual async Task<int> UpsertAsync(DriveTransferHistoryRecord item)
        {
            ...
            {
                upsertCommand.CommandText = "INSERT INTO driveTransferHistory (identityId,driveId,fileId,remoteIdentityId,status) " +
                                             "VALUES (@identityId,@driveId,@fileId,@remoteIdentityId,@status)"+
                ...
                upsertParam5.ParameterName = "@status";
                ...
                upsertParam5.Value = item.status;
        ...
        protected virtual async Task<int> UpdateAsync(DriveTransferHistoryRecord item)
        {
            ...
            await using var updateCommand = cn.CreateCommand();
            {
                ...
                updateParam5.ParameterName = "@status";
                ...
                updateParam5.Value = item.status;
        ...
        // SELECT identityId,driveId,fileId,remoteIdentityId,status
...
#pragma warning restore CS0168
            ...
            item.status = rdr.IsDBNull(4) ? throw new Exception("item is NULL, but set as NOT NULL") : (int)(long)rdr[4];
            return item;
       }
...
#pragma warning restore CS0168
        ...
        protected virtual async Task<DriveTransferHistoryRecord> GetAsync(Guid identityId,Guid driveId,Guid fileId,OdinId remoteIdentityId)
        {
            ...
            {
                get0Command.CommandText = "SELECT status FROM driveTransferHistory " +
...
#pragma warning restore CS0168
        ...
        protected virtual async Task<List<DriveTransferHistoryRecord>> GetAsync(Guid identityId,Guid driveId,Guid fileId)
        {
            ...
            {
                get1Command.CommandText = "SELECT remoteIdentityId,status FROM driveTransferHistory " +
                                             "WHERE identityId = @identityId AND driveId = @driveId AND fileId = @fileId;";
