...
namespace Odin.Core.Storage.Database.Identity.Table
{
    ...
    public abstract class TableKeyThreeValueCRUD : TableBase
    {
        ...
        protected TableKeyThreeValueCRUD(ScopedIdentityConnectionFactory scopedConnectionFactory)
        {
        ...
        protected virtual async Task<List<byte[]>> GetByKeyThreeAsync(Guid identityId,byte[] key3)
        {
            ...
            await using var get2Command = cn.CreateCommand();
            {
                get2Command.CommandText = "SELECT data FROM KeyThreeValue " +
                                             "WHERE identityId = @identityId AND "+(key3==null ? "key3 IS NULL" : "key3 = @key3") + " "+
                                             ";";
...
#pragma warning restore CS0168
        ...
        protected virtual async Task<List<KeyThreeValueRecord>> GetByKeyTwoThreeAsync(Guid identityId,byte[] key2,byte[] key3)
        {
            ...
            await using var get3Command = cn.CreateCommand();
            {
                get3Command.CommandText = "SELECT rowId,key1,data FROM KeyThreeValue " +
                                             "WHERE identityId = @identityId AND "+(key2==null ? "key2 IS NULL" : "key2 = @key2") + " AND "+(key3==null ? "key3 IS NULL" : "key3 = @key3") + " "+
                                             ";";
        ...
        protected virtual async Task<(List<KeyThreeValueRecord>, Int64? nextCursor)> PagingByRowIdAsync(int count, Guid identityId, Int64? inCursor)
        {
            if (count < 1)
