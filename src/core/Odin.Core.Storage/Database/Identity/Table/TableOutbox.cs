    ...
    : TableOutboxCRUD(cache, scopedConnectionFactory)
{
    ...
    public async Task<int> CheckInAsCancelledAsync(Guid checkOutStamp, UnixTimeUtc nextRunTime)
    {
        ...
        param1.Value = checkOutStamp.Cast(_scopedConnectionFactory.DatabaseType);
    ...
    public async Task<int> RecoverCheckedOutDeadItemsAsync(UnixTimeUtc pastThreshold)
    {
        ...
        param1.Value = SequentialGuid.CreateGuid(pastThreshold).Cast(_scopedConnectionFactory.DatabaseType); // UnixTimeMiliseconds
        param2.Value = identityKey.Cast(_scopedConnectionFactory.DatabaseType);
    ...
    public async Task<(int, int, UnixTimeUtc)> OutboxStatusDriveAsync(Guid driveId)
    {
        ...
        param1.Value = driveId.Cast(_scopedConnectionFactory.DatabaseType);
        param2.Value = identityKey.Cast(_scopedConnectionFactory.DatabaseType);
