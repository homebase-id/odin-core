...
#pragma warning restore CS0168
            ...
            item.key2NoLengthCheck = rdr.IsDBNull(2) ? throw new Exception("item is NULL, but set as NOT NULL") : (byte[])(rdr[2]);
            if (item.key2?.Length < 0)
                throw new Exception("Too little data in key2...");
            item.key3NoLengthCheck = rdr.IsDBNull(3) ? throw new Exception("item is NULL, but set as NOT NULL") : (byte[])(rdr[3]);
...
#pragma warning restore CS0168
            ...
            item.identityId = identityId;
            item.key1 = key1;
            item.key2NoLengthCheck = rdr.IsDBNull(0) ? throw new Exception("item is NULL, but set as NOT NULL") : (byte[])(rdr[0]);
            if (item.key2?.Length < 0)
                throw new Exception("Too little data in key2...");
            item.key3NoLengthCheck = rdr.IsDBNull(1) ? throw new Exception("item is NULL, but set as NOT NULL") : (byte[])(rdr[1]);
            if (item.key3?.Length < 0)
                throw new Exception("Too little data in key3...");
