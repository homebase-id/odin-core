...
{
    public class TableDriveMainIndexMigration2 : MigrationBase
    {
        public override int MigrationVersion => 2;
        public override int PreviousMigrationVersion => 0; // Fill it out with a real value

        private readonly ScopedIdentityConnectionFactory _scopedConnectionFactory;
        public TableDriveMainIndexMigration2(ScopedIdentityConnectionFactory scopedConnectionFactory)
        {
        ...
        public virtual async Task<int> EnsureTableExistsAsync(bool dropExisting = false)
        {
            ...
            if (dropExisting)
            {
                cmd.CommandText = "DROP TABLE IF EXISTS DriveMainIndexMigration2;";
            ...
            var wori = "";
            cmd.CommandText =
                "CREATE TABLE IF NOT EXISTS DriveMainIndexMigration2("
                   +rowid
                   ...
                   +$"){wori};"
                   +"CREATE INDEX IF NOT EXISTS Idx0DriveMainIndexMigration2 ON DriveMainIndexMigration2(identityId,driveId,fileSystemType,requiredSecurityGroup,created,rowId);"
                   +"CREATE INDEX IF NOT EXISTS Idx1DriveMainIndexMigration2 ON DriveMainIndexMigration2(identityId,driveId,fileSystemType,requiredSecurityGroup,modified,rowId);"
                   +"CREATE INDEX IF NOT EXISTS Idx2DriveMainIndexMigration2 ON DriveMainIndexMigration2(identityId,driveId,fileSystemType,requiredSecurityGroup,userDate,rowId);"
                   ;
        ...
        public async Task<int> CopyDataAsync(IConnectionWrapper cn)
        {
            ...
            {
                copyCommand.CommandText = "INSERT INTO DriveMainIndexMigration2 (rowId,identityId,driveId,fileId,globalTransitId,fileState,requiredSecurityGroup,fileSystemType,userDate,fileType,dataType,archivalStatus,historyStatus,senderId,groupId,uniqueId,byteCount,hdrEncryptedKeyHeader,hdrVersionTag,hdrAppData,hdrLocalVersionTag,hdrLocalAppData,hdrReactionSummary,hdrServerData,hdrTransferHistory,hdrFileMetaData,hdrTmpDriveAlias,hdrTmpDriveType,created,modified) " +
        ...
        // Will upgrade from the previous version to version 2
        public async Task UpAsync(IConnectionWrapper cn)
        {
            try
            {
                using (var trn = await cn.BeginStackedTransactionAsync())
                {
                    ...
                    if (await VerifyRowCount(cn, "DriveMainIndex", "DriveMainIndexMigration2") == false)
                    ...
                    await RenameAsync(cn, "DriveMainIndexMigration2", "DriveMainIndex");
                    trn.Commit();
                }
            }
            catch
        ...
        public async Task DownAsync(IConnectionWrapper cn)
        {
            try
            {
                using (var trn = await cn.BeginStackedTransactionAsync())
                {
                    ...
                    await RenameAsync(cn, "DriveMainIndex", "DriveMainIndexMigration2");
