...
{
    public TableFollowsMeMigrationV0(long previousVersion) : base(previousVersion)
    {
    }
    ...
    public async Task<int> CopyDataAsync(IConnectionWrapper cn)
    {
        ...
        await using var copyCommand = cn.CreateCommand();
        {
            copyCommand.CommandText =
                "INSERT INTO FollowsMeMigrationsV0 (rowId,identityId,identity,driveId,created,modified) " +
    ...
    public override async Task UpAsync(IConnectionWrapper cn)
    {
        using (var trn = await cn.BeginStackedTransactionAsync())
        {
            ...
            await SqlHelper.RenameAsync(cn, "FollowsMeMigrationsV0", "FollowsMe");
            trn.Commit();
        }
    }
