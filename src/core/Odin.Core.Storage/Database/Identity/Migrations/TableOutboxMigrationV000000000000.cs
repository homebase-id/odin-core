...
{
    public TableOutboxMigrationV0(long previousVersion) : base(previousVersion)
    {
    }
    ...
    public async Task<int> CopyDataAsync(IConnectionWrapper cn)
    {
        ...
        await using var copyCommand = cn.CreateCommand();
        {
            copyCommand.CommandText =
                "INSERT INTO OutboxMigrationsV0 (rowId,identityId,driveId,fileId,recipient,type,priority,dependencyFileId,checkOutCount,nextRunTime,value,checkOutStamp,correlationId,created,modified) " +
    ...
    public override async Task UpAsync(IConnectionWrapper cn)
    {
        using (var trn = await cn.BeginStackedTransactionAsync())
        {
            ...
            await SqlHelper.RenameAsync(cn, "OutboxMigrationsV0", "Outbox");
            trn.Commit();
        }
    }
    ...
    {
        await CheckSqlTableVersion(cn, "Outbox", MigrationVersion);
