...
{
    public TableRegistrationsMigrationV202508281508(long previousVersion) : base(previousVersion)
    {
    }

    public override long MigrationVersion => 202508281508;
    ...
    public async Task<int> CopyDataAsync(IConnectionWrapper cn)
    {
        ...
        await using var copyCommand = cn.CreateCommand();
        {
            copyCommand.CommandText =
                "INSERT INTO RegistrationsMigrationsV202508281508 (rowId,identityId,email,primaryDomainName,firstRunToken,disabled,markedForDeletionDate,planId,json,created,modified) " +
    ...
    public override async Task UpAsync(IConnectionWrapper cn)
    {
        ...
        using (var trn = await cn.BeginStackedTransactionAsync())
        {
            ...
            if (!await VerifyRowCount(cn, "Registrations", "RegistrationsMigrationsV202508281508"))
    ...
    public override async Task DownAsync(IConnectionWrapper cn)
    {
        ...
        using (var trn = await cn.BeginStackedTransactionAsync())
        {
            if (!await VerifyRowCount(cn, $"RegistrationsMigrationsV{PreviousVersion}", "Registrations"))
