...
namespace Odin.Core.Storage.Database.System.Table
{
    public record RegistrationsRecord
    {
        ...
        public Boolean enablePublicWebPresence { get; set; }
       ...
       */
        ...
        public virtual async Task<int> InsertAsync(RegistrationsRecord item)
        {
            ...
            await using var insertCommand = cn.CreateCommand();
            {
                ...
                insertCommand.CommandText = "INSERT INTO Registrations (identityId,email,primaryDomainName,firstRunToken,disabled,markedForDeletionDate,planId,enablePublicWebPresence,json,created,modified) " +
                                           $"VALUES (@identityId,@email,@primaryDomainName,@firstRunToken,@disabled,@markedForDeletionDate,@planId,@enablePublicWebPresence,@json,{sqlNowStr},{sqlNowStr})"+
                ...
                insertCommand.AddParameter("@enablePublicWebPresence", DbType.Boolean, item.enablePublicWebPresence);
        ...
        public virtual async Task<int> UpsertAsync(RegistrationsRecord item)
        {
            ...
            await using var upsertCommand = cn.CreateCommand();
            {
                ...
                upsertCommand.CommandText = "INSERT INTO Registrations (identityId,email,primaryDomainName,firstRunToken,disabled,markedForDeletionDate,planId,enablePublicWebPresence,json,created,modified) " +
                                            $"VALUES (@identityId,@email,@primaryDomainName,@firstRunToken,@disabled,@markedForDeletionDate,@planId,@enablePublicWebPresence,@json,{sqlNowStr},{sqlNowStr})"+
                                            "ON CONFLICT (identityId) DO UPDATE "+
                                            $"SET email = @email,primaryDomainName = @primaryDomainName,firstRunToken = @firstRunToken,disabled = @disabled,markedForDeletionDate = @markedForDeletionDate,planId = @planId,enablePublicWebPresence = @enablePublicWebPresence,json = @json,modified = {upsertCommand.SqlMax()}(Registrations.modified+1,{sqlNowStr}) "+
                                            "RETURNING created,modified,rowId;";
                ...
                upsertCommand.AddParameter("@enablePublicWebPresence", DbType.Boolean, item.enablePublicWebPresence);
        ...
        public virtual async Task<int> UpdateAsync(RegistrationsRecord item)
        {
            ...
            await using var updateCommand = cn.CreateCommand();
            {
                ...
                updateCommand.CommandText = "UPDATE Registrations " +
                                            $"SET email = @email,primaryDomainName = @primaryDomainName,firstRunToken = @firstRunToken,disabled = @disabled,markedForDeletionDate = @markedForDeletionDate,planId = @planId,enablePublicWebPresence = @enablePublicWebPresence,json = @json,modified = {updateCommand.SqlMax()}(Registrations.modified+1,{sqlNowStr}) "+
                ...
                updateCommand.AddParameter("@enablePublicWebPresence", DbType.Boolean, item.enablePublicWebPresence);
        ...
        // SELECT rowId,identityId,email,primaryDomainName,firstRunToken,disabled,markedForDeletionDate,planId,enablePublicWebPresence,json,created,modified
        ...
        public virtual async Task<RegistrationsRecord> PopAsync(Guid identityId)
        {
            ...
            await using var deleteCommand = cn.CreateCommand();
            {
                deleteCommand.CommandText = "DELETE FROM Registrations " +
                                             "WHERE identityId = @identityId " + 
                                             "RETURNING rowId,email,primaryDomainName,firstRunToken,disabled,markedForDeletionDate,planId,enablePublicWebPresence,json,created,modified";
...
#pragma warning restore CS0168
        ...
        public virtual async Task<RegistrationsRecord> GetAsync(Guid identityId)
        {
            ...
            {
                get0Command.CommandText = "SELECT rowId,email,primaryDomainName,firstRunToken,disabled,markedForDeletionDate,planId,enablePublicWebPresence,json,created,modified FROM Registrations " +
        ...
        public virtual async Task<(List<RegistrationsRecord>, Int64? nextCursor)> PagingByRowIdAsync(int count, Int64? inCursor)
        {
            if (count < 1)
            ...
            {
                getPaging0Command.CommandText = "SELECT rowId,identityId,email,primaryDomainName,firstRunToken,disabled,markedForDeletionDate,planId,enablePublicWebPresence,json,created,modified FROM Registrations " +
