...
namespace Odin.Core.Storage.SQLite.IdentityDatabase
{
    ...
    public class TableDriveCommandMessageQueueCRUD : TableBase
    {
        ...
        public virtual int TryInsert(DatabaseConnection conn, DriveCommandMessageQueueRecord item)
        {
            using (var _insertCommand = _database.CreateCommand())
            {
                _insertCommand.CommandText = "INSERT OR IGNORE INTO driveCommandMessageQueue (identityId,driveId,fileId,timeStamp) " +
                                             "VALUES (@identityId,@driveId,@fileId,@timeStamp)";
                var _insertParam1 = _insertCommand.CreateParameter();
                _insertParam1.ParameterName = "@identityId";
                _insertCommand.Parameters.Add(_insertParam1);
