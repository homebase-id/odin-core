...
public class SendReactionOutboxWorker(
    OutboxFileItem fileItem,
    ILogger<SendReactionOutboxWorker> logger,
...
) : OutboxWorkerBase(fileItem, logger)
{
    public async Task<(bool shouldMarkComplete, UnixTimeUtc nextRun)> Send(IOdinContext odinContext, DatabaseConnection cn, CancellationToken cancellationToken)
    {
        try
        {
            ...
            logger.LogDebug("SendReaction -> Sending request for file: {file} to {recipient}", FileItem.File, FileItem.Recipient);

            var globalTransitId = await HandleRequest(FileItem, cancellationToken);

            logger.LogDebug("SendReaction -> Success for gtid {gtid} (version:{version}) to {recipient} - Action: " +
    ...
    private async Task<Guid> HandleRequest(OutboxFileItem outboxItem, CancellationToken cancellationToken)
    {
        ...
        var request = OdinSystemSerializer.Deserialize<AddRemoteReactionRequest>(outboxItem.State.Data.ToStringFromUtf8Bytes());
        ...
        async Task<ApiResponse<PeerTransferResponse>> TrySendRequest()
        {
            var client = odinHttpClientFactory.CreateClientUsingAccessToken<IPeerReactionHttpClient>(
                recipient,
                clientAuthToken);

            return await client.AddReaction(request);
        }
