...
/// </summary>
public abstract class FileSystemStreamWriterBaseV2
{
    ...
    /// <summary />
    protected FileSystemStreamWriterBaseV2(IDriveFileSystem fileSystem, TenantContext tenantContext,
    ...
    public FileUploadPackageV2 Package { get; private set; }
    ...
    public virtual async Task StartUpload(UploadInstructionSet instructionSet, IOdinContext odinContext, IdentityDatabase db)
    {
        ...
        //get a new file id
        file = await FileSystem.Storage.CreateInternalFileId(driveId, db);
        ...
        this.Package = new FileUploadPackageV2(file, instructionSet!);
    ...
    public async Task<UploadResult> FinalizeUpload(IOdinContext odinContext, IdentityDatabase db)
    {
        ...
        if (metadata.AppData.UniqueId.HasValue)
        {
            ...
            var existingFile = await FileSystem.Query.GetFileByClientUniqueId(Package.InternalFile.DriveId, incomingClientUniqueId, odinContext, db);
        ...
        await ProcessNewFileUpload(Package, keyHeader, metadata, serverMetadata, odinContext, db);

        Dictionary<string, TransferStatus> recipientStatus = await ProcessTransitInstructions(Package, odinContext, db);
    ...
    protected abstract Task ValidateUnpackedData(FileUploadPackageV2 package, KeyHeader keyHeader, FileMetadata metadata, ServerMetadata serverMetadata,
        IOdinContext odinContext);

    /// <summary>
    /// Called when the incoming file does not exist on disk.  This is called after core validations are complete
    /// </summary>
    protected abstract Task ProcessNewFileUpload(FileUploadPackageV2 package, KeyHeader keyHeader, FileMetadata metadata, ServerMetadata serverMetadata,
    ...
    /// </summary>
    /// <returns></returns>
    protected abstract Task<Dictionary<string, TransferStatus>> ProcessTransitInstructions(FileUploadPackageV2 package, IOdinContext odinContext,
        IdentityDatabase db);
    ...
    /// </summary>
    /// <returns></returns>
    protected abstract Task<FileMetadata> MapUploadToMetadata(FileUploadPackageV2 package, UploadFileDescriptor uploadDescriptor, IOdinContext odinContext);

    protected virtual async Task<(KeyHeader keyHeader, FileMetadata metadata, ServerMetadata serverMetadata)> UnpackMetadata(FileUploadPackageV2 package,
        IOdinContext odinContext, IdentityDatabase db)
    {
        ...
        var metadataBytes = await FileSystem.Storage.GetAllFileBytesFromTempFile(package.TempMetadataFile, MultipartUploadParts.Metadata.ToString(), odinContext, db);
    ...
    protected async Task<Dictionary<string, TransferStatus>> ProcessTransitBasic(FileUploadPackageV2 package, FileSystemType fileSystemType,
        IOdinContext odinContext, IdentityDatabase db)
    {
    ...
    /// </summary>
    private async Task ValidateUploadCore(FileUploadPackageV2 package, KeyHeader keyHeader, FileMetadata metadata, ServerMetadata serverMetadata,
        IdentityDatabase db)
    {
