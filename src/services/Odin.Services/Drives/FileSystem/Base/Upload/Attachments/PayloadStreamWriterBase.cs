...
public abstract class PayloadStreamWriterBase
{
    ...
    public virtual async Task AddPayload(string key, string contentTypeFromMultipartSection, Stream data, IOdinContext odinContext)
    {
        ...
        var bytesWritten = await FileSystem.Storage.WriteUploadStream(_package.TempFile, extension, data, odinContext);
        if (bytesWritten > 0)
        {
        ...
        IOdinContext odinContext)
    {
        ...
        var bytesWritten = await FileSystem.Storage.WriteUploadStream(_package.TempFile, extenstion, data, odinContext);
    ...
    public async Task CleanupTempFiles(IOdinContext odinContext)
    {
        if (_package?.TempFile != null)
        {
            ...
            await FileSystem.Storage.CleanupUploadTemporaryFiles(_package.TempFile, uploadedPayloads, odinContext);
        }
    }
