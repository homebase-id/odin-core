...
namespace Odin.Services.Authentication.Owner
{
        ...
        TableKeyValueCached tblKeyValue)
    {
        ...
        private static readonly SingleKeyValueStorage PasswordDataStorage =
            TenantSystemStorage.CreateSingleKeyValueStorage(Guid.Parse(PasswordKeyContextKey));

        private const string EccKeyContextKey = "fb8d351e-2c4f-4f1d-9b84-aefe7bc11ee1";

        private static readonly SingleKeyValueStorage EccStorage =
            TenantSystemStorage.CreateSingleKeyValueStorage(Guid.Parse(EccKeyContextKey));
        ...
        public async Task ResetPasswordAsync(ResetPasswordRequest request, IOdinContext odinContext)
        {
            ...
            await this.AssertPasswordKeyMatchAsync(request.CurrentAuthenticationPasswordReply.NonceHashedPassword64,
                request.CurrentAuthenticationPasswordReply.Nonce64);
