...
2. The app (or server on its behalf) generates the **encryption keypair** and stores it **encrypted on the email drive**. **Chosen at implementation (2026-08-23): the SERVER generates and writes it**, seeded with optional client-collected entropy (the app's shake screen), because chat-kmp has no OpenPGP implementation and cryptography-kotlin cannot be seeded. The write order is load-bearing and enforced in `EmailAppService.GenerateKeyAsync`: **keyring durable on the drive first, certificate published second** - so "a certificate is published" implies "the keyring exists", and an app killed mid-setup can never strand a published key whose private half nobody holds. Drive storage puts it under the existing master-key/**Shamir recovery** umbrella automatically — losing the key means losing all encrypted mail, so recoverability is non-negotiable and this design gets it for free.
...
Configuration joins the `Email` section (DNS doc): Stalwart base address + admin credentials per host group (`Email:Stalwart:*`), plus its storage backends (PostgreSQL connection for metadata, S3 bucket for blobs - reusing the platform's existing S3 configuration pattern). **The PowerDNS-style caveat is discharged (2026-08-21)**: the management surface was live-verified against a dev Stalwart 0.16 before implementation - it is a JMAP-based registry (per-type methods like `x:Account/set`, variant `@type` discriminators), NOT a REST admin API, and `StalwartMailboxProvider` encodes the verified contract (reference: `docs/stalwart-admin-api-notes.md`; dev instance runbook: `docs/stalwart-dev-setup.md`). Two contract corrections from verification: app-password secrets are mail-server-GENERATED and returned exactly once (the wrapper returns them, never receives them), and Stalwart derives DKIM public keys from the installed private key, making the provisioned-key drift check a plain read-back comparison. The PostgreSQL+S3 cluster configuration remains unverified (dev instance runs RocksDB).
