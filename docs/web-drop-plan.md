...
- **On update, `Ttl` may only shorten.** Mirrors the `IsEncrypted` immutability guard at
  ...
  message.
- **First-read resolution must be set-only-if-still-negative**, so two parallel GETs cannot race, and
  must not bump `hdrVersionTag` or clients see a spurious change. Both to be verified during
  implementation.
