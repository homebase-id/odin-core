...
- **DKIM generation in Homebase, signing in Stalwart**: per-tenant keypair generated at email activation, TXT records written to the zone, key provisioned into Stalwart, which signs all outbound natively (see `docs/email-keys-plan.md`). Verify the chosen relay forwards pre-signed mail without requiring its own domain onboarding — outside-repo assumption.
