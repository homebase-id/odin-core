using System;
using System.Threading.Tasks;
using DotYou.IdentityRegistry;
using DotYou.Kernel.HttpClient;
using DotYou.Kernel.Services.Admin.IdentityManagement;
using DotYou.Kernel.Services.Circle;
using DotYou.Kernel.Services.Contacts;
using DotYou.Types;
using DotYou.Types.Circle;
using DotYou.Types.Identity;
using DotYou.Types.SignalR;
using Microsoft.AspNetCore.SignalR;
using Microsoft.Extensions.Logging;
using Newtonsoft.Json;

namespace DotYou.Kernel.Services.Demo
{
    public class PrototrialDemoDataService : DotYouServiceBase, IPrototrialDemoDataService
    {
        private readonly IPersonService _personService;
        private readonly IAdminIdentityAttributeService _admin;
        private readonly ICircleNetworkService _circleNetworkService;

        public PrototrialDemoDataService(DotYouContext context, ILogger logger, IPersonService personService, IAdminIdentityAttributeService admin, ICircleNetworkService circleNetworkService) : base(context, logger, null, null)
        {
            _personService = personService;
            _admin = admin;
            _circleNetworkService = circleNetworkService;
        }

        public async Task<bool> AddDigitalIdentities()
        {
            if (IsFrodo)
            {
                //sam was left out intentionally; like a blank page.  actually just so we can send a request and fulfill it.
                await _personService.Save(new Person() {DotYouId = (DotYouIdentity) "gandalf.middleearth.life", GivenName = "Olorin", Surname = "Maiar", Tag = "Fellowship", PrimaryEmail = "mail@gandalf.middleearth.life"});
                await _personService.Save(new Person() {DotYouId = (DotYouIdentity) "odin.valhalla.com", GivenName = "Odin", Surname = "Rune Bringer", Tag = "Acquaintance", PrimaryEmail = "mail@frodobaggins.me"});
            }

            if (IsSam)
            {
                //frodo was left out intentionally; like a blank page.  actually just so we can send a request and fulfill it.
                await _personService.Save(new Person() {DotYouId = (DotYouIdentity) "gandalf.middleearth.life", GivenName = "Olorin", Surname = "Maiar", Tag = "Fellowship", PrimaryEmail = "mail@gandalf.middleearth.life"});
                await _personService.Save(new Person() {DotYouId = (DotYouIdentity) "odin.valhalla.com", GivenName = "Odin", Surname = "Rune Bringer", Tag = "Acquaintance", PrimaryEmail = "mail@frodobaggins.me"});
            }

            if (IsGandalf)
            {
                await _personService.Save(new Person() {DotYouId = (DotYouIdentity) "frodobaggins.me", GivenName = "Frodo", Surname = "Baggins", Tag = "Fellowship", PrimaryEmail = "mail@frodobaggins.me"});
                await _personService.Save(new Person() {DotYouId = (DotYouIdentity) "samwisegamgee.me", GivenName = "Samwise", Surname = "Gamgee", Tag = "Fellowship", PrimaryEmail = "mail@samwisegamgee.me"});
                await _personService.Save(new Person() {DotYouId = (DotYouIdentity) "odin.valhalla.com", GivenName = "Odin", Surname = "Rune Bringer", Tag = "Acquaintance", PrimaryEmail = "mail@frodobaggins.me"});
            }

            if (IsOdin)
            {
                await _personService.Save(new Person() {DotYouId = (DotYouIdentity) "frodobaggins.me", GivenName = "Frodo", Surname = "Baggins", Tag = "Fellowship", PrimaryEmail = "mail@frodobaggins.me"});
                await _personService.Save(new Person() {DotYouId = (DotYouIdentity) "gandalf.middleearth.life", GivenName = "Olorin", Surname = "Maiar", Tag = "Fellowship", PrimaryEmail = "mail@gandalf.middleearth.life"});
                await _personService.Save(new Person() {DotYouId = (DotYouIdentity) "samwisegamgee.me", GivenName = "Samwise", Surname = "Gamgee", Tag = "Fellowship", PrimaryEmail = "mail@samwisegamgee.me"});
            }

            return true;
        }

        public async Task<bool> AddConnectionRequests()
        {
            // var toGandalf = new ConnectionRequestHeader()
            // {
            //     Id = Guid.NewGuid(),
            //     Message = "Connect with me!... and yes it's secret and safe",
            //     Recipient = (DotYouIdentity) "gandalf.middleearth.life"
            // };

            if (IsFrodo)
            {
                var toSam = new ConnectionRequestHeader()
                {
                    Id = Guid.NewGuid(),
                    Message = "Connect with me... we've got some walking to do",
                    Recipient = (DotYouIdentity) "samwisegamgee.me"
                };

                await _circleNetworkService.SendConnectionRequest(toSam);
            }

            if (IsSam)
            {
                var toFrodo = new ConnectionRequestHeader()
                {
                    Id = Guid.NewGuid(),
                    Message = "Connect with me..lets go for a long stroll",
                    Recipient = (DotYouIdentity) "frodobaggins.me"
                };

                await _circleNetworkService.SendConnectionRequest(toFrodo);
            }

            if (IsGandalf)
            {
                var toFrodo = new ConnectionRequestHeader()
                {
                    Id = Guid.NewGuid(),
                    Message = "Connect with me..lets go for a long stroll",
                    Recipient = (DotYouIdentity) "frodobaggins.me"
                };

                await _circleNetworkService.SendConnectionRequest(toFrodo);
            }

            if (IsOdin)
            {
                var toGandalf = new ConnectionRequestHeader()
                {
                    Id = Guid.NewGuid(),
                    Message = "Let us speak of Pantheons",
                    Recipient = (DotYouIdentity) "gandalf.middleearth.life"
                };

                await _circleNetworkService.SendConnectionRequest(toGandalf);
            }

            return true;
        }

        public async Task SetProfiles()
        {
            Profile profile = null;
            ConnectedProfile connectedProfile = null;
            if (IsFrodo)
            {
                profile = new Profile()
                {
                    Name = new NameAttribute()
                    {
                        Personal = "Frodo",
                        Surname = "Underhill",
                        Suffix = ""
                    },
                    AvatarUri = "https://i2.wp.com/a1.ec-images.myspacecdn.com/images01/20/849847696a4b587b3e848eddd93c207e/l.jpg?zoom=2"
                };

                connectedProfile = new ConnectedProfile()
                {
                    Name = new NameAttribute()
                    {
                        Personal = "Frodo",
                        Surname = "Baggins",
                        Suffix = "Ring-Bearer"
                    },
                    AvatarUri = "https://img.huffingtonpost.com/asset/5d022ceb240000300f8d12bb.jpeg?ops=scalefit_720_noupscale"
                };
            }

            if (IsSam)
            {
                profile = new Profile()
                {
                    Name = new NameAttribute()
                    {
                        Personal = "Samwise",
                        Surname = "Gamgee",
                        Suffix = "The Brave"
                    },
                    AvatarUri = "https://static.wikia.nocookie.net/middle-earth-film-saga/images/5/52/Sam_TTT_profile.png/revision/latest?cb=20190727211735"
                };
                
                connectedProfile = new ConnectedProfile()
                {
                    Name = new NameAttribute()
                    {
                        Personal = "Samwise",
                        Surname = "Gamgee",
                        Suffix = "Ring-bearer"
                    },
                    AvatarUri = "https://miro.medium.com/max/640/0*tEUhip6Bfy-dDZSD"
                };
            }

            if (IsGandalf)
            {
                profile = new Profile()
                {
                    Name = new NameAttribute()
                    {
                        Personal = "Gandalf",
                        Surname = "White",
                        Suffix = ""
                    },
                    AvatarUri = "data:image/jpeg;base64,"
                };
                
                connectedProfile = new ConnectedProfile()
                {
                    Name = new NameAttribute()
                    {
                        Personal = "Gandalf",
                        Surname = "White",
                        Suffix = "Maiar of Valinor"
                    },
                    AvatarUri = "https://static.wikia.nocookie.net/lotr/images/7/70/Gandalf%3B_The_White.jpg/revision/latest/scale-to-width-down/250?cb=20140110102343"
                };
            }

            if (IsOdin)
            {
                profile = new Profile()
                {
                    Name = new NameAttribute()
                    {
                        Personal = "Odin",
                        Surname = "",
                        Suffix = ""
                    },
                    AvatarUri = "https://static.wikia.nocookie.net/da27b848-42dd-4a68-933a-2ff32ed33895/scale-to-width/755"
                };
                
                connectedProfile = new ConnectedProfile()
                {
                    Name = new NameAttribute()
                    {
                        Personal = "Odin",
                        Surname = "",
                        Suffix = "Rune Bringer"
                    },
                    AvatarUri = "https://www.majesticdragonfly.com/image/cache/catalog/veronese/odin-with-ravens-norse-god-bust-statue-WU77529A4-900x900.jpg"
                };
            }

            await _admin.SavePublicProfile(profile);
            await _admin.SaveConnectedProfile(connectedProfile);
        }
        

        private bool IsFrodo => this.Context.HostDotYouId == "frodobaggins.me";
        private bool IsSam => this.Context.HostDotYouId == "samwisegamgee.me";
        private bool IsGandalf => this.Context.HostDotYouId == "gandalf.middleearth.life";
        private bool IsOdin => this.Context.HostDotYouId == "odin.valhalla.com";
    }
}