...
namespace DotYou.Kernel.Cryptography
{
    public static class CRC32C
    {
        ...
        /// <summary>
        /// Calculate the CRC32C checksum for a byte array.
        /// </summary>
        /// <param name="crc">The start value for the crc</param>
        /// <param name="buf">Pointer to the memory block</param>
        public static UInt32 crc32c(UInt32 crc, byte[] buf)
        {
            crc = crc ^ ~0U; //0xFFFFFFFF

            for (int i = 0, iTop = buf.Length; i < iTop; i++)
    ...
    public class RsaKeyMgmt
    {
        ...
        public void generateNewKey(UInt64 hours)
        {
            ...
            currentKey.crc32c = CRC32C.crc32c(0, Encoding.ASCII.GetBytes(currentKey.publicKey));
