...
public class CircleMembershipAppApiClient : AppApiTestUtils
{
    private readonly AppClientToken _token;

    public CircleMembershipAppApiClient(OwnerApiTestUtils ownerApiTestUtils, AppClientToken token) : base(ownerApiTestUtils)
    {
        _token = token;
    }
    ...
    public async Task<ApiResponse<List<CircleDomainResult>>> GetDomainsInCircle(Guid circleId)
    {
        ...
        {
            var svc = RefitCreator.RestServiceFor<ICircleMembershipHttpClient>(client,  _token.SharedSecret);
