...
//
//         var commentFileHeader = await frodoOwnerClient.Drive.GetTextReactionHeader(commentUploadResult.File);
//
...
//
//         var feedbackFileHeader = await frodoOwnerClient.Drive.GetFileHeader(commentUploadResult.File);
//
//         Assert.IsTrue(feedbackFileHeader.FileId == commentUploadResult.File.FileId);
//         Assert.IsTrue(feedbackFileHeader.FileMetadata.AppData.JsonContent == comment);
//         Assert.IsTrue(feedbackFileHeader.FileMetadata.ReferencedFile == uploadedContentResult.File, "target reference file not referenced");
//
//         // Get the target file and validate reaction was updated
//
//         var targetFileHeader = await frodoOwnerClient.Drive.GetFileHeader(uploadedContentResult.File);
...
//
//     [Test]
//     [Description("Tests that the owner can react with an emoji to their post")]
//     public async Task CanUploadEmojiReactionOwnerDrive()
//     {
//         var frodoOwnerClient = _scaffold.CreateOwnerApiClient(TestIdentities.Frodo);
//
//         //create a channel drive
...
//
//         //
//         // Frodo posts reaction to his post
//         //
//         var feedbackContent = "Indeed, Indeed I am Mr. Underhill";
//         var targetReferenceFile = uploadResult.File;
//         var feedbackUploadResult = await UploadReaction(frodoOwnerClient, frodoChannelDrive, targetReferenceFile, feedbackContent, false);
//
//         Assert.IsTrue(uploadResult.File.TargetDrive == feedbackUploadResult.File.TargetDrive);
//
//         var feedbackFileHeader = await frodoOwnerClient.Drive.GetFileHeader(feedbackUploadResult.File);
//
//         Assert.IsTrue(feedbackFileHeader.FileId == feedbackUploadResult.File.FileId);
//         Assert.IsTrue(feedbackFileHeader.FileMetadata.AppData.JsonContent == feedbackContent);
//         Assert.IsTrue(feedbackFileHeader.FileMetadata.ReferencedFile == targetReferenceFile, "target reference file not referenced");
//         Assert.IsTrue(feedbackFileHeader.ServerMetadata.SystemFileType == SystemFileType.Reaction);
//     }
//
//
...
//     }
//
//     private async Task<UploadResult> UploadReaction(OwnerApiClient client, TargetDrive targetDrive, ExternalFileIdentifier targetFile, string feedbackContent, bool allowDistribution)
//     {
