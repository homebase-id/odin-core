...
namespace Odin.Core.Storage.Tests.IdentityDatabaseTests
{
    public class TableMainIndexTests
    {
        ...
        public void GetSizeTest()
        {
            ...
            db.AddEntry(driveId, f1, Guid.NewGuid(), 1, 1, s1, t1, null, 42, new UnixTimeUtc(0), 0, null, null, 1);
            db.AddEntry(driveId, f3, Guid.NewGuid(), 1, 1, s1, t1, null, 42, new UnixTimeUtc(0), 2, null, null, 2);
            db.AddEntry(driveId, f2, Guid.NewGuid(), 1, 1, s1, t1, null, 42, new UnixTimeUtc(0), 1, null, null, 3);
            db.AddEntry(driveId, f5, Guid.NewGuid(), 1, 1, s1, t1, null, 42, new UnixTimeUtc(0), 3, null, null, 4);
            db.AddEntry(driveId, f4, Guid.NewGuid(), 1, 1, s1, t1, null, 42, new UnixTimeUtc(0), 2, null, null, 5);

            var (count, size) = db.tblDriveMainIndex.GetDriveSize(driveId);
        ...
        public void CannotInsertZeroSizeTest()
        {
            ...
            try
            {
                db.AddEntry(driveId, f1, Guid.NewGuid(), 1, 1, s1, t1, null, 42, new UnixTimeUtc(0), 0, null, null, 0);
            }
            catch (Exception ex)
            {
        ...
        public void UpdateRowTest()
        {
            ...
            db.tblDriveMainIndex.UpdateRow(driveId, k1, fileType: 8);
            md = db.tblDriveMainIndex.Get(driveId, k1);
            if (md.fileType != 8)
                Assert.Fail();

            db.tblDriveMainIndex.UpdateRow(driveId, k1, dataType: 43);
            md = db.tblDriveMainIndex.Get(driveId, k1);
            if (md.dataType != 43)
                Assert.Fail();

            var sid2 = "frodo.baggins".ToUtf8ByteArray();
            db.tblDriveMainIndex.UpdateRow(driveId, k1, senderId: sid2);
            md = db.tblDriveMainIndex.Get(driveId, k1);
            if (ByteArrayUtil.EquiByteArrayCompare(sid2, md.senderId.ToUtf8ByteArray()) == false)
                Assert.Fail();

            var tid2 = Guid.NewGuid();
            db.tblDriveMainIndex.UpdateRow(driveId, k1, groupId: tid2);
            md = db.tblDriveMainIndex.Get(driveId, k1);
            ...
            db.tblDriveMainIndex.UpdateRow(driveId, k1, nullableUniqueId: kludge);
            md = db.tblDriveMainIndex.Get(driveId, k1);
            ...
            db.tblDriveMainIndex.UpdateRow(driveId, k1, globalTransitId: gtid2);
            md = db.tblDriveMainIndex.Get(driveId, k1);
            ...
            db.tblDriveMainIndex.UpdateRow(driveId, k1, userDate: ud2);
            md = db.tblDriveMainIndex.Get(driveId, k1);
            if (ud2 != md.userDate)
                Assert.Fail();

            db.tblDriveMainIndex.UpdateRow(driveId, k1, requiredSecurityGroup: 55);
            md = db.tblDriveMainIndex.Get(driveId, k1);
            Assert.True(md.requiredSecurityGroup == 55);

            db.tblDriveMainIndex.UpdateRow(driveId, k1, byteCount: 42);
            md = db.tblDriveMainIndex.Get(driveId, k1);
