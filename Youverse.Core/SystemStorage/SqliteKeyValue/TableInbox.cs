...
namespace Youverse.Core.SystemStorage.SqliteKeyValue
{
    ...
    public class TableInbox : TableKeyValueBase  // Make it IDisposable??
    {
            ...
            using (var cmd = _keyValueDatabase.CreateCommand())
            {
                ...
                cmd.CommandText =
                    @"CREATE TABLE inbox(
                     ...
                     value BLOB,
                     popstamp BLOB); "
                    + "CREATE INDEX timestampidx ON inbox(timestamp);"
                    + "CREATE INDEX boxidx ON inbox(boxid);"
                    + "CREATE INDEX popidx ON inbox(popstamp);";
        ...
        public InboxItem Get(byte[] fileId)
        {
            lock (_selectLock)
            {
                ...
                using (SQLiteDataReader rdr = _selectCommand.ExecuteReader(System.Data.CommandBehavior.SingleRow))
                {
                    ...
                    item.timeStamp = (UInt64) rdr.GetInt64(1);
        ...
        public void InsertRow(byte[] boxId, byte[] fileId, int priority, byte[] value)
        {
            lock (_insertLock)
            {
                ...
                _iparam3.Value = UnixTime.GetUnixTimeSeconds();
            ...
            lock (_popLock)
            {
                ...
                using (SQLiteDataReader rdr = _popCommand.ExecuteReader(System.Data.CommandBehavior.Default))
                {
                    ...
                    while (rdr.Read())
                    {
                        ...
                        item.timeStamp = (UInt64)rdr.GetInt64(2);
        ...
        /// </summary>
        public void PopRecoverDead(UInt64 UnixTimeSeconds)
        {
            lock (_popLock)
            {
                if (_popRecoverCommand == null)
                ...
                _pcrecoverparam1.Value = SequentialGuid.CreateGuid(UnixTimeSeconds*1000); // UnixTimeMiliseconds
