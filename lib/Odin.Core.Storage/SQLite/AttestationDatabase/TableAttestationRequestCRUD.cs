...
namespace Odin.Core.Storage.SQLite.AttestationDatabase
{
    ...
    public class TableAttestationRequestCRUD : TableBase
    {
        ...
        public virtual int Insert(AttestationRequestRecord item)
        {
            lock (_insertLock)
            {
                if (_insertCommand == null)
                {
                    ...
                    _insertCommand.CommandText = "INSERT INTO attestationRequest (identity,requestEnvelope,timestamp) " +
                                                 "VALUES ($identity,$requestEnvelope,$timestamp)";
        ...
        public virtual int Upsert(AttestationRequestRecord item)
        {
            lock (_upsertLock)
            {
                if (_upsertCommand == null)
                {
                    ...
                    _upsertCommand.CommandText = "INSERT INTO attestationRequest (identity,requestEnvelope,timestamp) " +
                                                 "VALUES ($identity,$requestEnvelope,$timestamp)"+
        ...
        // SELECT identity,requestEnvelope,timestamp
...
#pragma warning restore CS0168
        ...
        public int Delete(string identity)
        {
            ...
            lock (_delete0Lock)
            {
                ...
                if (count > 0)
                    _cache.Remove("TableAttestationRequestCRUD", identity.ToString());
                return count;
            } // Lock
        }

        public AttestationRequestRecord ReadRecordFromReader0(SqliteDataReader rdr, string identity)
        {
            if (identity == null) throw new Exception("Cannot be null");
            if (identity?.Length < 0) throw new Exception("Too short");
            if (identity?.Length > 65535) throw new Exception("Too long");
...
#pragma warning restore CS0168
        ...
        public AttestationRequestRecord Get(string identity)
        {
            if (identity == null) throw new Exception("Cannot be null");
            if (identity?.Length < 0) throw new Exception("Too short");
            if (identity?.Length > 65535) throw new Exception("Too long");
            var (hit, cacheObject) = _cache.Get("TableAttestationRequestCRUD", identity.ToString());
            if (hit)
            ...
            lock (_get0Lock)
            {
                ...
                using (SqliteDataReader rdr = _database.ExecuteReader(_get0Command, System.Data.CommandBehavior.SingleRow))
                {
                    if (!rdr.Read())
                    {
                        _cache.AddOrUpdate("TableAttestationRequestCRUD", identity.ToString(), null);
                        return null;
                    }
                    var r = ReadRecordFromReader0(rdr, identity);
                    _cache.AddOrUpdate("TableAttestationRequestCRUD", identity.ToString(), r);
                    return r;
                } // using
            } // lock
        }
