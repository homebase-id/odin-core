    ...
    public class TableAppNotificationsCRUD : TableBase
    {
        ...
        public virtual int Insert(AppNotificationsRecord item)
        {
            lock (_insertLock)
            {
                if (_insertCommand == null)
                {
                    ...
                    _insertCommand.CommandText = "INSERT INTO AppNotifications (notificationId,unread,senderId,data,created,modified) " +
                                                 "VALUES ($notificationId,$unread,$senderId,$data,$created,$modified)";
                ...
                _insertParam3.Value = item.senderId?.DomainName ?? (object)DBNull.Value;
                _insertParam4.Value = item.data ?? (object)DBNull.Value;
                var now = UnixTimeUtcUnique.Now();
                _insertParam5.Value = now.uniqueTime;
                item.modified = null;
        ...
        public virtual int Upsert(AppNotificationsRecord item)
        {
            lock (_upsertLock)
            {
                if (_upsertCommand == null)
                {
                    ...
                    _upsertCommand.CommandText = "INSERT INTO AppNotifications (notificationId,unread,senderId,data,created) " +
                                                 "VALUES ($notificationId,$unread,$senderId,$data,$created)"+
                                                 "ON CONFLICT (notificationId) DO UPDATE "+
                                                 "SET unread = $unread,senderId = $senderId,data = $data,modified = $modified "+
                ...
                _upsertParam3.Value = item.senderId?.DomainName ?? (object)DBNull.Value;
        ...
        public virtual int Update(AppNotificationsRecord item)
        {
            lock (_updateLock)
            {
                ...
                _updateParam3.Value = item.senderId?.DomainName ?? (object)DBNull.Value;
        ...
        // SELECT notificationId,unread,senderId,data,created,modified
...
#pragma warning restore CS0168
            ...
            {
                item.senderId = new OdinId(rdr.GetString(2));
            }
...
#pragma warning restore CS0168
            ...
            {
                item.senderId = (OdinId)rdr.GetString(1);
            }
        ...
        public AppNotificationsRecord Get(Guid notificationId)
        {
            ...
            lock (_get0Lock)
            {
                if (_get0Command == null)
                {
                    ...
                    _get0Command.CommandText = "SELECT unread,senderId,data,created,modified FROM AppNotifications " +
        ...
        public List<AppNotificationsRecord> PagingByCreated(int count, UnixTimeUtcUnique? inCursor, out UnixTimeUtcUnique? nextCursor)
        {
            ...
            lock (_getPaging5Lock)
            {
                if (_getPaging5Command == null)
                {
                    ...
                    _getPaging5Command.CommandText = "SELECT notificationId,unread,senderId,data,created,modified FROM AppNotifications " +
                ...
                using (SqliteDataReader rdr = _database.ExecuteReader(_getPaging5Command, System.Data.CommandBehavior.Default))
                {
                    var result = new List<AppNotificationsRecord>();
                    int n = 0;
