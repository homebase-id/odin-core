...
#pragma warning restore CS0168
        ...
        public List<TagIndexRecord> PagingByRowid(int count, Int32? inCursor, out Int32? nextCursor)
        {
            if (count < 1)
            ...
            lock (_getPaging0Lock)
            {
                if (_getPaging0Command == null)
                {
                    ...
                    _getPaging0Command.CommandText = "SELECT rowid,fileId,tagId FROM tagIndex " +
