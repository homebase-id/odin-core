...
namespace Youverse.Core.Tests.SystemStorageTests.KeyValue
{
    public class TableOutboxTests
    {
        ...
        public void PopRecoverDeadTest()
        {
            ...
            db.tblOutbox.PopRecoverDead(UnixTime.GetUnixTimeSeconds()+2);
        ...
        public void DualBoxTest()
        {
            ...
            db.tblOutbox.InsertRow(b2, f3, 10, v1);
            db.tblOutbox.InsertRow(b2, f4, 10, v1);
            db.tblOutbox.InsertRow(b2, f5, 10, v1);
        ...
        public void ExampleTest()
        {
            ...
            db.tblOutbox.PopRecoverDead(60*10);
        ...
        public void PopAllTest()
        {
            ...
            if (SequentialGuid.muidcmp(r[0].fileId, f1) != 0)
                Assert.Fail();
            if (SequentialGuid.muidcmp(r[0].boxId, box1id) != 0)
                Assert.Fail();

            if (SequentialGuid.muidcmp(r[1].fileId, f4) != 0)
                Assert.Fail();
            if (SequentialGuid.muidcmp(r[1].boxId, box2id) != 0)
                Assert.Fail();

            if (SequentialGuid.muidcmp(r[2].fileId, f6) != 0)
                Assert.Fail();
            if (SequentialGuid.muidcmp(r[2].boxId, box3id) != 0)
                Assert.Fail();
