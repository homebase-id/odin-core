using System;
using System.Threading.Tasks;
using Newtonsoft.Json;
using Youverse.Core.Services.Base;

namespace Youverse.Core.Services.Container.Query.LiteDb
{
    public class LiteDbContainerMetaDataIndexer : IContainerMetadataIndexer
    {
        private readonly ISystemStorage _systemStorage;
        private readonly IContainerResolver _containerResolver;

        public LiteDbContainerMetaDataIndexer(IContainerResolver containerResolver, ISystemStorage systemStorage)
        {
            _containerResolver = containerResolver;
            _systemStorage = systemStorage;
        }

        public async Task Rebuild()
        {
            var page = await _containerResolver.GetContainers(new PageOptions(1, Int32.MaxValue));

            //TODO: optimize by making this parallelly processed or something
            foreach (var container in page.Results)
            {
                RebuildInternal(container);
            }
        }

        public async Task Rebuild(Guid containerId)
        {
            var container = await _containerResolver.Resolve(containerId);
            await RebuildInternal(container);
        }
        
        private async Task RebuildInternal(ContainerInfo container)
        {
            var item = new IndexedItem()
            {
                Id = Guid.NewGuid(),
                CategoryId = Guid.NewGuid(),
                CreatedTimestamp = DateTimeExtensions.UnixTimeMilliseconds(),
                LastUpdatedTimestamp = DateTimeExtensions.UnixTimeMilliseconds(),
                FileId = Guid.NewGuid(),
                JsonPayload = JsonConvert.SerializeObject(new {message = "Suzy's boots", previewImage = "data:image/jpeg;base64,"})
            };

            _systemStorage.WithTenantSystemStorage<IndexedItem>(container.IndexName, s => s.Save(item));
        }
    }
}