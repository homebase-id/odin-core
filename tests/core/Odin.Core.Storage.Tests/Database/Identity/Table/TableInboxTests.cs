...
namespace Odin.Core.Storage.Tests.Database.Identity.Table
{
    public class TableInboxTests : IocTestBase
    {
        ...
        public async Task PagingByRowIdTest(DatabaseType databaseType)
        {
            await RegisterServicesAsync(databaseType);
            await using var scope = Services.BeginLifetimeScope();
            var tbl = scope.Resolve<TableInbox>();
            var identityKey = scope.Resolve<OdinIdentity>();
            ...
            await tbl.InsertAsync(new InboxRecord() { identityId = identityKey, boxId = boxId, fileId = f1, priority = 0, value = null });
            await tbl.InsertAsync(new InboxRecord() { identityId = identityKey, boxId = boxId, fileId = f2, priority = 1, value = null });
            await tbl.InsertAsync(new InboxRecord() { identityId = identityKey, boxId = boxId, fileId = f3, priority = 2, value = null });
