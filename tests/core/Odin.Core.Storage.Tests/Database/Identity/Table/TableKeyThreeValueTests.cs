...
namespace Odin.Core.Storage.Tests.Database.Identity.Table
{
    public class TableKeyThreeValueTests : IocTestBase
    {
        ...
        public async Task PagingByRowIdTest(DatabaseType databaseType)
        {
            await RegisterServicesAsync(databaseType);
            await using var scope = Services.BeginLifetimeScope();
            var tbl = scope.Resolve<TableKeyThreeValue>();
            ...
            var key3 = SequentialGuid.CreateGuid().ToByteArray();

            await tbl.InsertAsync(new KeyThreeValueRecord() { key1 = k1, key2 = key2, key3 = key3, data = null });
            await tbl.InsertAsync(new KeyThreeValueRecord() { key1 = k2, key2 = key2, key3 = key3, data = null });
            await tbl.InsertAsync(new KeyThreeValueRecord() { key1 = k3, key2 = key2, key3 = key3, data = null });
