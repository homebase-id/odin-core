...
public class TableLastSeenTests : IocTestBase
{
    ...
    [TestCase(DatabaseType.Postgres)]
#endif
    public async Task ItShouldDeleteOldRecords(DatabaseType databaseType)
    {
        await RegisterServicesAsync(databaseType);
    ...
    public async Task ItShouldPageByRowId(DatabaseType databaseType)
    {
        ...
        var (page1, cursor1) = await tableLastSeen.PagingByRowIdAsync(2, null);
        Assert.That(page1.Count, Is.EqualTo(2));
        Assert.That(cursor1, Is.Not.Null);

        var (page2, cursor2) = await tableLastSeen.PagingByRowIdAsync(2, cursor1);
        Assert.That(page2.Count, Is.EqualTo(1));
        Assert.That(cursor2, Is.Null);

        var (all, allCursor) = await tableLastSeen.PagingByRowIdAsync(100, null);
