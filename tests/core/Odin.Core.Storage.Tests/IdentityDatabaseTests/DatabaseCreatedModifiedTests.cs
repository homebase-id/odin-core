...
namespace Odin.Core.Storage.Tests.IdentityDatabaseTests
{
    public class DatabaseCreatedModifiedTests
    {
        ...
        [Test]
        public async Task InsertTimersTest()
        {
            ...
            using (var myc = db.CreateDisposableConnection())
            {
                ...
                try
                {
                    n = await db.tblConnections.InsertAsync(item1);
                    Debug.Assert(n == 0);
                }
                catch (Exception)
                {
                ...
                var loaded = await db.tblConnections.GetAsync(new OdinId("frodo.baggins.me"));
        ...
        // Using the connections table just because it happens to have FinallyAddCreatedModified();
        [Test]
        public async Task UpdateTimersTest()
        {
            ...
            using (var myc = db.CreateDisposableConnection())
            {
                ...
                await db.tblConnections.InsertAsync(item1);
                // We don't need to validate Insert, we did that above.

                var copyCreated = item1.created;
                Thread.Sleep(1000);
                await db.tblConnections.UpdateAsync(item1);
        ...
        // Using the connections table just because it happens to have FinallyAddCreatedModified();
        [Test]
        public async Task UpsertTimersTest()
        {
