    ...
    public class TableAppNotificationsTest
    {
        ...
        public void InsertGetTest()
        {
            ...
            using (var myc = db.CreateDisposableConnection())
            {
                ...
                var i = db.tblAppNotificationsTable.Insert(myc, new AppNotificationsRecord() { notificationId = nid, senderId = (OdinId)"frodo.com", unread = 1, data = d1 });
                Debug.Assert(i == 1);
                var r = db.tblAppNotificationsTable.Get(myc, nid);
                Debug.Assert(r != null);
        ...
        public void InsertPageTest()
        {
            ...
            using (var myc = db.CreateDisposableConnection())
            {
                ...
                var i = db.tblAppNotificationsTable.Insert(myc, new AppNotificationsRecord() { notificationId = nid, senderId = (OdinId)"frodo.com", unread = 1, data = d1 });
                Debug.Assert(i == 1);
                i = db.tblAppNotificationsTable.Insert(myc, new AppNotificationsRecord() { notificationId = nid2, senderId = (OdinId)"frodo.com", unread = 1, data = d1 });
                Debug.Assert(i == 1);

                var results = db.tblAppNotificationsTable.PagingByCreated(myc, 1, null, out var cursor2);

                Debug.Assert(results.Count == 1);
                Debug.Assert(cursor2 != null);
                Debug.Assert(cursor2.Value.uniqueTime == results[0].created.uniqueTime);

                results = db.tblAppNotificationsTable.PagingByCreated(myc, 1, cursor2, out cursor2);
