    ...
    public class TableCronTests
    {
        ...
        public void ExampleCronTest()
        {
            ...
            using (var myc = db.CreateDisposableConnection())
            {
                ...
                // first
                //
                db.tblCron.Upsert(myc, new CronRecord() { identityId = frodo, type = 1, data = d1 });
                db.tblCron.Upsert(myc, new CronRecord() { identityId = sam, type = 1, data = d1 });
                ...
                var il1 = db.tblCron.Pop(myc, 10);
                Assert.True(il1.Count == 2);

                // Let's say that you finished everything for Frodo correctly:
                db.tblCron.PopCommitList(myc, new List<Guid>() { il1[0].identityId });

                // Let's say Sam's job failed and needs to run again later
                db.tblCron.PopCancelList(myc, new List<Guid>() { il1[1].identityId });
                ...
                db.tblCron.PopRecoverDead(myc, t);
            }
        }
        ...
        public void UpsertCron02Test()
        {
            ...
            using (var myc = db.CreateDisposableConnection())
            {
                ...
                t1 = new UnixTimeUtc();
                db.tblCron.Upsert(myc, new CronRecord() { identityId = c1, type = 1, data = d2 });
                t2 = new UnixTimeUtc();

                i = db.tblCron.Get(myc, c1, 1);
                Assert.True(ByteArrayUtil.muidcmp(c1, i.identityId) == 0);
                Assert.True(i.type == 1);
                Assert.True(i.runCount == 0);
                Assert.True((i.nextRun >= t1) && (i.nextRun <= t2));
        ...
        public void InsertCron04Test()
        {
            ...
            using (var myc = db.CreateDisposableConnection())
            {
                ...
                var t1 = new UnixTimeUtc();
                db.tblCron.Upsert(myc, new CronRecord() { identityId = c1, type = 1, data = d1 });
                db.tblCron.Upsert(myc, new CronRecord() { identityId = c2, type = 1, data = d1 });
                var t2 = new UnixTimeUtc();

                var i1 = db.tblCron.Get(myc, c1, 1);
                ...
                var i2 = db.tblCron.Get(myc, c2, 1);
        ...
        public void PopCron01Test()
        {
            ...
            using (var myc = db.CreateDisposableConnection())
            {
                ...
                db.tblCron.Upsert(myc, new CronRecord() { identityId = i1, type = 1, data = d1 });
                db.tblCron.Upsert(myc, new CronRecord() { identityId = i2, type = 1, data = d1 });

                var il1 = db.tblCron.Pop(myc, 1);
                Assert.True(il1.Count == 1);

                var il2 = db.tblCron.Pop(myc, 1);
        ...
        public void CronRecoverDead02Test()
        {
            ...
            using (var myc = db.CreateDisposableConnection())
            {
                ...
                var t1 = new UnixTimeUtc();
                db.tblCron.Upsert(myc, new CronRecord() { identityId = Guid.NewGuid(), type = 1, data = d1 });
                db.tblCron.Upsert(myc, new CronRecord() { identityId = Guid.NewGuid(), type = 1, data = d1 });
                db.tblCron.Upsert(myc, new CronRecord() { identityId = Guid.NewGuid(), type = 1, data = d1 });
                var t2 = new UnixTimeUtc();
                ...
                var il2 = db.tblCron.Pop(myc, 10);
                Assert.True(il2.Count == 0);

                db.tblCron.Upsert(myc, new CronRecord() { identityId = Guid.NewGuid(), type = 1, data = d1 });
                db.tblCron.Upsert(myc, new CronRecord() { identityId = Guid.NewGuid(), type = 1, data = d1 });

                il1 = db.tblCron.Pop(myc, 10);
                Assert.True(il1.Count == 2);
                var t4 = new UnixTimeUtc();

                il2 = db.tblCron.Pop(myc, 10);
                Assert.True(il2.Count == 0);

                db.tblCron.PopRecoverDead(myc, t3);
                ...
                db.tblCron.PopRecoverDead(myc, t5);
        ...
        public void CronCommit01Test()
        {
            ...
            using (var myc = db.CreateDisposableConnection())
            {
                ...
                db.tblCron.PopCommitList(myc, new List<Guid>() { il1[0].identityId, il1[1].identityId });

                Thread.Sleep(1);

                db.tblCron.PopRecoverDead(myc, UnixTimeUtc.Now());
        ...
        public void CronCancel01Test()
        {
            ...
            using (var myc = db.CreateDisposableConnection())
            {
                ...
                db.tblCron.PopCancelList(myc, new List<Guid>() { il1[0].identityId, il1[1].identityId });
        ...
        public void CronTimer01Test()
        {
            ...
            using (var myc = db.CreateDisposableConnection())
            {
                ...
                db.tblCron.PopCancelList(myc, new List<Guid>() { il1[0].identityId });


                // Pop it, now counters are incremented
                il1 = db.tblCron.Pop(myc, 1);
                Assert.True(il1.Count == 1);
