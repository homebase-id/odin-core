    ...
    public class DriveIndexDatabaseTests
    {
        ...
        public async Task CursorsEmpty01Test()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                var (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 10, cursor, requiredSecurityGroup: allIntRange);
                Debug.Assert(refCursor.pagingCursor == null);
                Debug.Assert(refCursor.stopAtBoundary == null);
                Debug.Assert(refCursor.pagingCursor == null);
                Debug.Assert(moreRows == false);

                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 10, refCursor, requiredSecurityGroup: allIntRange);
                Debug.Assert(refCursor.pagingCursor == null);
                Debug.Assert(refCursor.stopAtBoundary == null);
                Debug.Assert(refCursor.pagingCursor == null);
                Debug.Assert(moreRows == false);
                ...
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 10, inCursor, requiredSecurityGroup: allIntRange);
        ...
        public async Task CursorsBatch02Test()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                var (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 100, cursor, requiredSecurityGroup: allIntRange);
                ...
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 100, refCursor, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 0);
        ...
        public async Task CursorsBatch03Test()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                QueryBatchCursor cursor = null;
                var (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 2, cursor, requiredSecurityGroup: allIntRange);
                ...
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 2, refCursor, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 2);
                Debug.Assert(refCursor.stopAtBoundary == null);
                Debug.Assert(ByteArrayUtil.muidcmp(f5.ToByteArray(), refCursor.nextBoundaryCursor) == 0);
        ...
        public async Task CursorsBatch05Test()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                for (int i = 1; i < 100; i++)
                {
                    (result, moreRows, cursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 2, cursor, requiredSecurityGroup: allIntRange);
                    c += result.Count;
                    if (result.Count == 0)
                        break;
                }
                ...
                for (int i = 1; i < 100; i++)
                {
                    (result, moreRows, var refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 2, cursor, requiredSecurityGroup: allIntRange);
                    c += result.Count;
                    if (result.Count == 0)
                        break;
                }
        ...
        public async Task CursorsBoundaryTest01()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                var (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAsync(driveId, 100, cursor, newestFirstOrder: false, fileIdSort: true, requiredSecurityGroup: allIntRange);
        ...
        public async Task CursorsUDBoundaryTest01()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                var (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAsync(driveId, 100, cursor, newestFirstOrder: false, fileIdSort: false, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 3);
                Debug.Assert(cursor.nextBoundaryCursor == null);
        ...
        public async Task CursorsBoundaryTest02()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                QueryBatchCursor cursor = new QueryBatchCursor(f4.ToByteArray());
                var (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAsync(driveId, 100, cursor, newestFirstOrder: true, fileIdSort: true, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 3);
                Debug.Assert(cursor.nextBoundaryCursor == null);
                Debug.Assert(moreRows == false);
        ...
        public async Task CursorsUDBoundaryTest02()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                var (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAsync(driveId, 100, cursor, newestFirstOrder: true, fileIdSort: false, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 3);
                Debug.Assert(cursor.nextBoundaryCursor == null);
        ...
        public async Task FileStateTest()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                QueryBatchCursor cursor = null;
                var (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 10, cursor, requiredSecurityGroup: allIntRange, fileStateAnyOf: new List<Int32>() { 0 });
                Debug.Assert(result.Count == 0);
                Debug.Assert(moreRows == false);

                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 10, cursor, requiredSecurityGroup: allIntRange, fileStateAnyOf: new List<Int32>() { 3 });
                Debug.Assert(result.Count == 1);
                Debug.Assert(moreRows == false);

                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 10, cursor, requiredSecurityGroup: allIntRange, fileStateAnyOf: new List<Int32>() { 1, 2 });
                ...
                (result, moreRows, var outc2) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 10, c2, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 1);
                Debug.Assert(moreRows == false);
                ...
                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 10, cursor, fileStateAnyOf: new List<Int32>() { 42, 43 }, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 2);
                Debug.Assert(moreRows == false);
            }
        }
        ...
        public async Task ArchivalStatusTest()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 10, cursor, requiredSecurityGroup: allIntRange, archivalStatusAnyOf: new List<Int32>() { 0 });
                Debug.Assert(result.Count == 3);
                Debug.Assert(moreRows == false);

                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 10, cursor, requiredSecurityGroup: allIntRange, archivalStatusAnyOf: new List<Int32>() { 1 });
                Debug.Assert(result.Count == 2);
                Debug.Assert(moreRows == false);

                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 10, cursor, requiredSecurityGroup: allIntRange, archivalStatusAnyOf: new List<Int32>() { 2 });
                Debug.Assert(result.Count == 1);
                Debug.Assert(moreRows == false);

                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 10, cursor, archivalStatusAnyOf: new List<Int32>() { 0, 1 }, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 5);
                Debug.Assert(moreRows == false);

                UnixTimeUtcUnique c2 = new UnixTimeUtcUnique(0), outc2 = new UnixTimeUtcUnique(0);
                (result, moreRows, outc2) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 10, c2, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 0);
                Debug.Assert(moreRows == false);
                ...
                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 10, cursor, archivalStatusAnyOf: new List<Int32>() { 0 }, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 2);
                Debug.Assert(moreRows == false);
        ...
        public async Task QueryBatchCursorNewestHasRows01()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                QueryBatchCursor cursor = null;
                var (result, hasRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAsync(driveId, 2, cursor, newestFirstOrder: true, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 2);
                Debug.Assert(hasRows == true);

                (result, hasRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAsync(driveId, 1, refCursor, newestFirstOrder: true, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 1);
        ...
        public async Task QueryBatchUserDateCursorNewestHasRows01()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                QueryBatchCursor cursor = null;
                var (result, hasRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAsync(driveId, 2, cursor, newestFirstOrder: true, fileIdSort: false, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 2);
                Debug.Assert(hasRows == true);

                (result, hasRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAsync(driveId, 1, refCursor, newestFirstOrder: true, fileIdSort: false, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 1);
                Debug.Assert(hasRows == false);
        ...
        public async Task QueryBatchCursorOldestHasRows01()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                QueryBatchCursor cursor = null;
                var (result, hasRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAsync(driveId, 2, cursor, newestFirstOrder: false, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 2);
                Debug.Assert(hasRows == true);

                (result, hasRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAsync(driveId, 1, refCursor, newestFirstOrder: false, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 1);
                Debug.Assert(hasRows == false);
        ...
        public async Task QueryBatchUserDateCursorOldestHasRows01()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                QueryBatchCursor cursor = null;
                var (result, hasRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAsync(driveId, 2, cursor, newestFirstOrder: false, fileIdSort: false, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 2);
                Debug.Assert(hasRows == true);

                (result, hasRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAsync(driveId, 1, refCursor, newestFirstOrder: false, fileIdSort: false, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 1);
                Debug.Assert(hasRows == false);
        ...
        public async Task QueryBatchCursorOldestNewest01()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                QueryBatchCursor cursor = null;
                var (result, hasRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAsync(driveId, 1, cursor, newestFirstOrder: true, requiredSecurityGroup: allIntRange);
                Debug.Assert(ByteArrayUtil.muidcmp(result[0], f3) == 0);

                cursor = null;
                (result, hasRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAsync(driveId, 1, cursor, newestFirstOrder: false, requiredSecurityGroup: allIntRange);
        ...
        public async Task TestQueryBatchStartPointGuid()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                var (result, hasRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAsync(driveId, 10, cursor, newestFirstOrder: true, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 2);
                Debug.Assert(hasRows == false);
                Debug.Assert(ByteArrayUtil.muidcmp(result[0], f2) == 0);
                ...
                (result, hasRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAsync(driveId, 10, cursor, newestFirstOrder: false, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 3);
                Debug.Assert(hasRows == false);
        ...
        public async Task TestQueryBatchUserDateStartPointTime()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                // Get all the newest items. We should get f2, f1 and no more because f3 is the start point.
                var (result, hasRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAsync(driveId, 10, cursor, newestFirstOrder: true, fileIdSort: false, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 3);
                Debug.Assert(hasRows == false);
                Debug.Assert(ByteArrayUtil.muidcmp(result[0], f5) == 0);
                Debug.Assert(ByteArrayUtil.muidcmp(result[1], f1) == 0);
                ...
                // Get all the oldest items. We should get f4,f5,f6 because f3 is the start point and we're getting oldest first.
                (result, hasRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAsync(driveId, 10, cursor, newestFirstOrder: false, fileIdSort: false, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 2);
                Debug.Assert(hasRows == false);
                Debug.Assert(ByteArrayUtil.muidcmp(result[0], f4) == 0);
        ...
        public async Task TestQueryBatchStopBoundaryGuid()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                // Get all the newest items. We should get f6,f5,f4 and no more because f3 is the boundary.
                (result, hasRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAsync(driveId, 10, cursor, newestFirstOrder: true, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 0);
                Debug.Assert(hasRows == false);
                Debug.Assert(ByteArrayUtil.muidcmp(cursor.pagingCursor, f4.ToByteArray()) == 0);
        ...
        public async Task CursorsModified01Test()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                var (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 100, cursor, requiredSecurityGroup: allIntRange);
                ...
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 100, cursor, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 0); // Nothing in the DB should be modified
                Debug.Assert(outCursor.uniqueTime == 0);
                Debug.Assert(moreRows == false);
            }
        ...
        public async Task CursorsModified02Test()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                UnixTimeUtcUnique cursor = UnixTimeUtcUnique.ZeroTime;
                var (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 2, cursor, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 0);
                Debug.Assert(moreRows == false);

                // Modify one item make sure we can get it.
                await _testDatabase.tblDriveMainIndex.TestTouchAsync(driveId, f2);
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 2, outCursor, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 1);
                Debug.Assert(ByteArrayUtil.muidcmp(result[0], f2) == 0);
        ...
        public async Task RequiredSecurityGroupBatch01Test()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                var (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 5);
                Debug.Assert(moreRows == false);

                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 0, end: 0));
                Debug.Assert(result.Count == 1);
                Debug.Assert(moreRows == false);

                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 1, end: 1));
                Debug.Assert(result.Count == 1);
                Debug.Assert(moreRows == false);

                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 2, end: 2));
                Debug.Assert(result.Count == 2);
                Debug.Assert(moreRows == false);

                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 3, end: 3));
                Debug.Assert(result.Count == 1);
                Debug.Assert(moreRows == false);

                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 4, end: 10));
                Debug.Assert(result.Count == 0);
                Debug.Assert(moreRows == false);

                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 1, end: 2));
                Debug.Assert(result.Count == 3);
                Debug.Assert(moreRows == false);
            }
        }
        ...
        public async Task RequiredSecurityGroupModified02Test()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                var (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, inCursor, requiredSecurityGroup: allIntRange);
                ...
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, inCursor, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 5); // Ensure everything is now "modified"
                Debug.Assert(moreRows == false);

                outCursor = UnixTimeUtcUnique.ZeroTime;
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, inCursor, requiredSecurityGroup: new IntRange(start: 0, end: 0));
                Debug.Assert(result.Count == 1);
                Debug.Assert(moreRows == false);

                outCursor = UnixTimeUtcUnique.ZeroTime;
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, inCursor, requiredSecurityGroup: new IntRange(start: 1, end: 1));
                Debug.Assert(result.Count == 1);
                Debug.Assert(moreRows == false);

                outCursor = UnixTimeUtcUnique.ZeroTime;
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, inCursor, requiredSecurityGroup: new IntRange(start: 2, end: 2));
                Debug.Assert(result.Count == 2);
                Debug.Assert(moreRows == false);

                outCursor = UnixTimeUtcUnique.ZeroTime;
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, inCursor, requiredSecurityGroup: new IntRange(start: 3, end: 3));
                Debug.Assert(result.Count == 1);
                Debug.Assert(moreRows == false);

                outCursor = UnixTimeUtcUnique.ZeroTime;
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, inCursor, requiredSecurityGroup: new IntRange(start: 2, end: 3));
                Debug.Assert(result.Count == 3);
                Debug.Assert(moreRows == false);
            }
        }
        ...
        public async Task SecurityGroupAndAclBatch01Test()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: allIntRange, aclAnyOf: new List<Guid>() { a4 });
                Debug.Assert(result.Count == 2);
                Debug.Assert(moreRows == false);

                // For NO valid security group, and a valid ACL, just the valid ACLs
                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 0, end: 0), aclAnyOf: new List<Guid>() { a1 });
                Debug.Assert(result.Count == 0);
                Debug.Assert(moreRows == false);
                ...
                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 1, end: 1), aclAnyOf: new List<Guid>() { a1 });
                Debug.Assert(result.Count == 1);
                Debug.Assert(moreRows == false);

                // For security Group 1 or any of the ACLs a3, a4 we have 3
                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 1, end: 1), aclAnyOf: new List<Guid>() { a3, a4 });
                Debug.Assert(result.Count == 0);
                Debug.Assert(moreRows == false);
        ...
        public async Task SecurityGroupAndAclBatch02Test()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: allIntRange, aclAnyOf: new List<Guid>() { a2 });
                Debug.Assert(result.Count == 3);
                Debug.Assert(moreRows == false);

                // ACL: Any security group, circles a1, a2. We should have 4 (two with a2, one with a1, one with no circles)
                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: allIntRange, aclAnyOf: new List<Guid>() { a1, a2 });
                Debug.Assert(result.Count == 4);
                Debug.Assert(moreRows == false);

                // ACL: Any security group, circles a5. We should have 1 (none with a5, one with no circles)
                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: allIntRange, aclAnyOf: new List<Guid>() { a5 });
                Debug.Assert(result.Count == 1);
                Debug.Assert(moreRows == false);
                ...
                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 0, end: 0), aclAnyOf: new List<Guid>() { a4 });
                Debug.Assert(result.Count == 0);
                Debug.Assert(moreRows == false);

                // ACL: No security group, circles a2. We should have none
                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 0, end: 0), aclAnyOf: new List<Guid>() { a2 });
                Debug.Assert(result.Count == 0);
                Debug.Assert(moreRows == false);

                // ACL: No security group, circles a1, a2. We should have none
                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 0, end: 0), aclAnyOf: new List<Guid>() { a1, a2 });
                Debug.Assert(result.Count == 0);
                Debug.Assert(moreRows == false);

                // ACL: No security group, circles a5. We should have none
                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 0, end: 0), aclAnyOf: new List<Guid>() { a5 });
                Debug.Assert(result.Count == 0);
                Debug.Assert(moreRows == false);
                ...
                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 2, end: 2), aclAnyOf: new List<Guid>() { a2 });
                Debug.Assert(result.Count == 2);
                Debug.Assert(moreRows == false);

                // ACL: Security group 1, circles a4. We should have 0 (none with a4, none with circles)
                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 1, end: 1), aclAnyOf: new List<Guid>() { a4 });
                Debug.Assert(result.Count == 0);
                Debug.Assert(moreRows == false);

                // ACL: Security group 1, circles a1, a2. We should have 2
                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 1, end: 1), aclAnyOf: new List<Guid>() { a1, a2 });
                Debug.Assert(result.Count == 2);
                Debug.Assert(moreRows == false);

                // ACL: Security group 2, circles a1, a2. We should have 2
                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 2, end: 2), aclAnyOf: new List<Guid>() { a1, a2 });
                Debug.Assert(result.Count == 2);
                Debug.Assert(moreRows == false);

                // ACL: Security group 1, circles a5. We should have 0 (none with a5, none with circles)
                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 1, end: 1), aclAnyOf: new List<Guid>() { a5 });
                Debug.Assert(result.Count == 0);
                Debug.Assert(moreRows == false);

                // ACL: Security group 2, circles a5. We should have 1 (none with a5, one with no circles)
                cursor = null;
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 2, end: 2), aclAnyOf: new List<Guid>() { a5 });
                Debug.Assert(result.Count == 1);
                Debug.Assert(moreRows == false);
        ...
        public async Task SecurityGroupAndAclBatch02ModifiedTest()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                var (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, cursor, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 5);
                Debug.Assert(moreRows == false);

                // ACL: Security group 1, no circles. We should have 2 entries
                cursor = new UnixTimeUtcUnique(0);
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 1, end: 1));
                Debug.Assert(result.Count == 2);
                Debug.Assert(moreRows == false);

                // ACL: Security group 0, no circles. We should have 0 entries
                cursor = new UnixTimeUtcUnique(0);
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 0, end: 0));
                Debug.Assert(result.Count == 0);
                Debug.Assert(moreRows == false);
                ...
                // ACL: Any security group, circles a4. We should have 2 (one with a4, one with no circles)
                cursor = new UnixTimeUtcUnique(0);
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, cursor, requiredSecurityGroup: allIntRange, aclAnyOf: new List<Guid>() { a4 });
                Debug.Assert(result.Count == 2);
                Debug.Assert(moreRows == false);

                // ACL: Any security group, circles a2. We should have 3 (two with a2, one with no circles)
                cursor = new UnixTimeUtcUnique(0);
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, cursor, requiredSecurityGroup: allIntRange, aclAnyOf: new List<Guid>() { a2 });
                Debug.Assert(result.Count == 3);
                Debug.Assert(moreRows == false);

                // ACL: Any security group, circles a1, a2. We should have 4 (two with a2, one with a1, one with no circles)
                cursor = new UnixTimeUtcUnique(0);
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, cursor, requiredSecurityGroup: allIntRange, aclAnyOf: new List<Guid>() { a1, a2 });
                Debug.Assert(result.Count == 4);
                Debug.Assert(moreRows == false);

                // ACL: Any security group, circles a5. We should have 1 (none with a5, one with no circles)
                cursor = new UnixTimeUtcUnique(0);
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, cursor, requiredSecurityGroup: allIntRange, aclAnyOf: new List<Guid>() { a5 });
                Debug.Assert(result.Count == 1);
                Debug.Assert(moreRows == false);
                ...
                // ACL: No security group, circles a4. We should have none
                cursor = new UnixTimeUtcUnique(0);
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 0, end: 0), aclAnyOf: new List<Guid>() { a4 });
                Debug.Assert(result.Count == 0);
                Debug.Assert(moreRows == false);

                // ACL: No security group, circles a2. We should have none
                cursor = new UnixTimeUtcUnique(0);
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 0, end: 0), aclAnyOf: new List<Guid>() { a2 });
                Debug.Assert(result.Count == 0);
                Debug.Assert(moreRows == false);

                // ACL: No security group, circles a1, a2. We should have none
                cursor = new UnixTimeUtcUnique(0);
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 0, end: 0), aclAnyOf: new List<Guid>() { a1, a2 });
                Debug.Assert(result.Count == 0);
                Debug.Assert(moreRows == false);

                // ACL: No security group, circles a5. We should have none
                cursor = new UnixTimeUtcUnique(0);
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 0, end: 0), aclAnyOf: new List<Guid>() { a5 });
                Debug.Assert(result.Count == 0);
                Debug.Assert(moreRows == false);
                ...
                // ACL: One security group 2, circles a2. We should have 2 (one with a2, one with no circles)
                cursor = new UnixTimeUtcUnique(0);
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 2, end: 2), aclAnyOf: new List<Guid>() { a2 });
                Debug.Assert(result.Count == 2);
                Debug.Assert(moreRows == false);

                // ACL: Security group 1, circles a4. We should have 0 (none with a4, none with circles)
                cursor = new UnixTimeUtcUnique(0);
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 1, end: 1), aclAnyOf: new List<Guid>() { a4 });
                Debug.Assert(result.Count == 0);
                Debug.Assert(moreRows == false);

                // ACL: Security group 1, circles a1, a2. We should have 2
                cursor = new UnixTimeUtcUnique(0);
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 1, end: 1), aclAnyOf: new List<Guid>() { a1, a2 });
                Debug.Assert(result.Count == 2);
                Debug.Assert(moreRows == false);

                // ACL: Security group 2, circles a1, a2. We should have 2
                cursor = new UnixTimeUtcUnique(0);
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 2, end: 2), aclAnyOf: new List<Guid>() { a1, a2 });
                Debug.Assert(result.Count == 2);
                Debug.Assert(moreRows == false);

                // ACL: Security group 1, circles a5. We should have 0 (none with a5, none with circles)
                cursor = new UnixTimeUtcUnique(0);
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 1, end: 1), aclAnyOf: new List<Guid>() { a5 });
                Debug.Assert(result.Count == 0);
                Debug.Assert(moreRows == false);

                // ACL: Security group 2, circles a5. We should have 1 (none with a5, one with no circles)
                cursor = new UnixTimeUtcUnique(0);
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 400, cursor, requiredSecurityGroup: new IntRange(start: 2, end: 2), aclAnyOf: new List<Guid>() { a5 });
                Debug.Assert(result.Count == 1);
                Debug.Assert(moreRows == false);
        ...
        public async Task GlobalTransitId05Test()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                var (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 1, cursor, globalTransitIdAnyOf: new List<Guid>() { t1 }, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 0);
                Debug.Assert(moreRows == false);

                // Now we should be able to find it
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 1, cursor, globalTransitIdAnyOf: new List<Guid>() { t1, g1 }, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 1);
                Debug.Assert(moreRows == false);

                UnixTimeUtcUnique inCursor = UnixTimeUtcUnique.ZeroTime, outCursor = UnixTimeUtcUnique.ZeroTime;
                await _testDatabase.tblDriveMainIndex.TestTouchAsync(driveId, f1); // Make sure we can find it
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 1, inCursor, globalTransitIdAnyOf: new List<Guid>() { t1, g1 }, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 1);
                Debug.Assert(moreRows == false);
            }
        }
        ...
        public async Task UniqueId05Test()
        {
            ...
            using (var myc = _testDatabase.CreateDisposableConnection())
            {
                ...
                cursor = null;
                // We shouldn't be able to find any like this:
                var (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 1, cursor, uniqueIdAnyOf: new List<Guid>() { t1 }, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 0);
                Debug.Assert(moreRows == false);

                // Now we should be able to find it
                (result, moreRows, refCursor) = await _testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 1, cursor, uniqueIdAnyOf: new List<Guid>() { t1, u1 }, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 1);
                Debug.Assert(moreRows == false);

                UnixTimeUtcUnique inCursor = UnixTimeUtcUnique.ZeroTime, outCursor = UnixTimeUtcUnique.ZeroTime;
                await _testDatabase.tblDriveMainIndex.TestTouchAsync(driveId, f1); // Make sure we can find it
                (result, moreRows, outCursor) = await _testDatabase.metaIndex.QueryModifiedAsync(driveId, 1, inCursor, uniqueIdAnyOf: new List<Guid>() { t1, u1 }, requiredSecurityGroup: allIntRange);
                Debug.Assert(result.Count == 1);
                Debug.Assert(moreRows == false);
            }
        }
        ...
        public async Task AddEntryTest()
        {
            ...
            var (result, moreRows, refCursor) = await testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, cursor, requiredSecurityGroup: allIntRange);
            ...
            (result, moreRows, refCursor) = await testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 400, refCursor, requiredSecurityGroup: allIntRange);
            Debug.Assert(result.Count == 400);
            Debug.Assert(moreRows == true);
            ...
            (result, moreRows, outCursor) = await testDatabase.metaIndex.QueryModifiedAsync(driveId, 100, inCursor, requiredSecurityGroup: allIntRange);
            Debug.Assert(result.Count == 0);
            Debug.Assert(moreRows == false);
            ...
            (result, moreRows, outCursor) = await testDatabase.metaIndex.QueryModifiedAsync(driveId, 100, outCursor, requiredSecurityGroup: allIntRange);
            Debug.Assert(result.Count == 1);
            ...
            //
            cursor = null;
            (result, moreRows, refCursor) = await testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 1, cursor, requiredSecurityGroup: allIntRange);
            Debug.Assert(result.Count == 1);
            Debug.Assert(moreRows == true);
            ...
            (result, moreRows, refCursor) = await testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 1, refCursor, requiredSecurityGroup: allIntRange);
            Debug.Assert(result.Count == 1);
            Debug.Assert(moreRows == true);
            if (true)
            {
            ...
            //
            cursor = null;
            (result, moreRows, refCursor) = await testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 1, cursor, filetypesAnyOf: new List<int>() { 0, 4 }, requiredSecurityGroup: allIntRange);
            Debug.Assert(moreRows == true);
            Debug.Assert(result.Count >= 1);
            ...
            //
            cursor = null;
            (result, moreRows, refCursor) = await testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 100, cursor,
            ...
            //
            cursor = null;
            (result, moreRows, refCursor) = await testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 1, cursor,
            ...
            //
            cursor = null;
            (result, moreRows, refCursor) = await testDatabase.metaIndex.QueryBatchAutoAsync(driveId, 10,
                cursor,
