...
{
    //
    // TryLockAsync leans on an implementation detail of Nito's AsyncLock (it takes a free lock
    // synchronously and only looks at the cancellation token when it would have to queue).
    // These three tests exist to fail loudly if that ever stops being true, because the callers
    // that matter - the TLS handshake path - would otherwise silently stop issuing certificates.
    //

    [Test]
    public async Task TryLockAsync_AcquiresWhenFree()
    {
        var keyedMutex = new KeyedAsyncLock();

        var handle = await keyedMutex.TryLockAsync("free-key");

        Assert.That(handle, Is.Not.Null, "TryLockAsync must acquire a lock that nobody holds");
        handle!.Dispose();
    ...
    public async Task TryLockAsync_ReturnsNullWhenHeld_AndDoesNotWait()
    {
        ...
        using (await keyedMutex.LockAsync("held-key"))
        {
            var sw = System.Diagnostics.Stopwatch.StartNew();
            var handle = await keyedMutex.TryLockAsync("held-key");
            sw.Stop();

            Assert.That(handle, Is.Null, "TryLockAsync must not acquire a lock somebody else holds");
            Assert.That(sw.ElapsedMilliseconds, Is.LessThan(1000), "TryLockAsync must not wait");
        }
    }

    [Test]
    public async Task TryLockAsync_AcquiresAgainAfterRelease_AndLeaksNoState()
    {
        var keyedMutex = new KeyedAsyncLock();

        using (await keyedMutex.LockAsync("cycle-key"))
        {
            Assert.That(await keyedMutex.TryLockAsync("cycle-key"), Is.Null);
        }

        var handle = await keyedMutex.TryLockAsync("cycle-key");
        Assert.That(handle, Is.Not.Null);
        handle!.Dispose();

        // A failed TryLockAsync must not leave a ref-counted entry behind
        Assert.That(keyedMutex.Count, Is.EqualTo(0));
    }

    [Test]
    public async Task TryLockAsync_DoesNotBlockOtherKeys()
    {
        var keyedMutex = new KeyedAsyncLock();

        using (await keyedMutex.LockAsync("key-a"))
        {
            var handle = await keyedMutex.TryLockAsync("key-b");
            Assert.That(handle, Is.Not.Null);
            handle!.Dispose();
        }
    }
