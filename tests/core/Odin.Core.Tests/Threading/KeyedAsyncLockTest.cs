...
public class KeyedAsyncLockTests
{
    ...
    public async Task TryRunWithLockAsync_RunsAction_AndReturnsTrue_WhenFree()
    {
        ...
        var observedCountInside = -1;
        var result = await keyedMutex.TryRunWithLockAsync("k", () =>
        {
            observedCountInside = keyedMutex.Count;
            return Task.CompletedTask;
        });

        Assert.That(result, Is.True);
        Assert.That(observedCountInside, Is.EqualTo(1), "Lock should be held while the action runs.");
        Assert.That(keyedMutex.Count, Is.EqualTo(0), "Lock should be released after the action.");
    }

    [Test]
    public async Task TryRunWithLockAsync_SkipsAction_WhenHeldByLockAsync_AndDoesNotLeakRefCount()
    {
        var keyedMutex = new KeyedAsyncLock();
        var key = "k";
        ...
        using (await keyedMutex.LockAsync(key))
        {
            ...
            var ran = false;
            var result = await keyedMutex.TryRunWithLockAsync(key, () =>
            {
                ran = true;
                return Task.CompletedTask;
            });

            Assert.That(result, Is.False, "Should not acquire a held lock.");
            Assert.That(ran, Is.False, "Action must not run when the lock is held.");

            // A failed try must not bump the refcount.
            Assert.That(keyedMutex.Count, Is.EqualTo(1), "Failed try must not increment the refcount.");
        }

        // Holder released and the failed try left nothing behind.
        Assert.That(keyedMutex.Count, Is.EqualTo(0));
    }
    ...
    public async Task TryRunWithLockAsync_ConcurrentSameKey_OnlyOneRuns()
    {
        ...
        var first = keyedMutex.TryRunWithLockAsync(key, async () =>
        {
        ...
        var secondRan = false;
        var secondResult = await keyedMutex.TryRunWithLockAsync(key, () =>
        {
            secondRan = true;
            return Task.CompletedTask;
        });
    ...
    public async Task TryRunWithLockAsync_SucceedsAgain_AfterRelease()
    {
        ...
        Assert.That(await keyedMutex.TryRunWithLockAsync(key, () => Task.CompletedTask), Is.True);
        Assert.That(await keyedMutex.TryRunWithLockAsync(key, () => Task.CompletedTask), Is.True);
        Assert.That(keyedMutex.Count, Is.EqualTo(0));
    }

    [Test]
    public async Task TryRunWithLockAsync_HeldLock_DoesNotBlockSubsequentLockAsync_OnDifferentKey()
    {
        var keyedMutex = new KeyedAsyncLock();

        using var a = await keyedMutex.LockAsync("a");
        var result = await keyedMutex.TryRunWithLockAsync("b", () => Task.CompletedTask);

        Assert.That(result, Is.True, "Distinct keys must not block each other.");
    }
    ...
    public async Task TryRunWithLockAsync_ActionThrows_PropagatesAndReleasesLock()
    {
        ...
        Assert.ThrowsAsync<InvalidOperationException>(async () =>
            await keyedMutex.TryRunWithLockAsync(key, () => throw new InvalidOperationException("boom")));

        // Lock released despite the throw -> count back to zero and re-runnable.
        Assert.That(keyedMutex.Count, Is.EqualTo(0));
        Assert.That(await keyedMutex.TryRunWithLockAsync(key, () => Task.CompletedTask), Is.True);
    }
    ...
    public void TryRunWithLockAsync_CanceledToken_Throws()
    {
        ...
        Assert.ThrowsAsync<OperationCanceledException>(async () =>
            await keyedMutex.TryRunWithLockAsync("k", () =>
            {
                ran = true;
                return Task.CompletedTask;
            }, cts.Token));
        ...
        Assert.That(keyedMutex.Count, Is.EqualTo(0), "A canceled try must not register a lock.");
    }
}
