...
/// original drained twice in a row (<c>…_NotConnected</c> does) the two collapse to one, since the
/// second drain of the same tenant with nothing in between is a no-op.</item>
...
/// <item>The private drive-create / connect / follow / upload / overwrite / assert helpers are
/// <see cref="DataSubscriptionScenario"/>; they were duplicated across all four fixtures in this
/// folder.</item>
/// <item>The connection and follow calls are asserted, in
/// <see cref="DataSubscriptionScenario.ConnectAsync"/> and
/// <see cref="DataSubscriptionScenario.FollowAsync"/>. The V1 owner clients asserted success inside the
/// client (<c>CircleNetworkApiClient.SendConnectionRequestTo</c> and friends), so this keeps the
/// coverage rather than adding to it. Follow answers <b>204 NoContent</b>, not 200.</item>
...
public class DataSubscriptionAndDistributionTests1 : V2Fixture
{
    ...
    public async Task CanUploadStandardFileThenDeleteThenDistributeDeletion()
    {
        ...
        await DataSubscriptionScenario.AssertFeedDriveHasFileAsync(samOwnerClient,
            DataSubscriptionScenario.FeedQueryByFileType(fileType), uploadedContent, standardFileUploadResult);
    ...
    public async Task CommentsAreNotDistributed()
    {
        ...
        await DataSubscriptionScenario.AssertFeedDriveHasFileAsync(samOwnerClient,
            DataSubscriptionScenario.FeedQueryByFileType(standardFileType), uploadedContent, standardFileUploadResult);
    ...
    public async Task ReactionSummaryIsDistributedWhenCommentAddedByOwnerToStandardUnencryptedFile_WhenConnected()
    {
        ...
        //
        await DataSubscriptionScenario.ConnectAsync(frodoOwnerClient, samOwnerClient, securedChannelCircleId);
        ...
        await DataSubscriptionScenario.AssertFeedDriveHasFileAsync(samOwnerClient, standardFileQuery, uploadedContent,
            standardFileUploadResult);
    ...
    public async Task ReactionSummaryIsDistributedWhenCommentAdded_ByAnotherConnectedIdentity_ToStandardEncryptedFile_AndJsonContentIsEmpty()
    {
        ...
        // Sam should have the blog post from frodo in Sam's feed
        await DataSubscriptionScenario.AssertFeedDriveHasFileAsync(samOwnerClient, standardFileQuery,
            encryptedStandardFileJsonContent64, standardFileUploadResult);
        ...
        // Sam should, however, have a reaction summary update for that comment on the original file
        //
        var theFile2 = await DataSubscriptionScenario.AssertFeedDriveHasFileAsync(samOwnerClient, standardFileQuery,
            encryptedStandardFileJsonContent64, standardFileUploadResult);
    ...
    public async Task UnencryptedStandardFile_UploadedByOwner_Distributed_ToFollowers_That_AreNotConnected()
    {
        ...
        var theFile = await DataSubscriptionScenario.AssertFeedDriveHasFileAsync(samOwnerClient,
            DataSubscriptionScenario.FeedQueryByGlobalTransitId(uploadResult), uploadedContent, uploadResult);
    ...
    {
        var theFile = await DataSubscriptionScenario.AssertFeedDriveHasFileAsync(follower, standardFileQuery,
            expectedContent, standardFileUploadResult);
