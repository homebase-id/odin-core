...
public class UniversalDriveApiClientV2(OdinId identity, IApiClientFactory factory)
{
        ...
        FileSystemType fileSystemType = FileSystemType.Standard)
    {
        ...
        var client = factory.CreateHttpClient(identity, out var sharedSecret, fileSystemType);
        {
            ...
            var driveSvc = RestService.For<IUniversalDriveV2HttpClientApi>(client);
            ApiResponse<UploadResult> response = await driveSvc.CreateFile(parts.ToArray());
    ...
    public async Task<ApiResponse<InboxStatus>> ProcessInbox(TargetDrive drive, int batchSize = 1)
    {
        ...
        var transitSvc = RefitCreator.RestServiceFor<IUniversalDriveV2HttpClientApi>(client, sharedSecret);
    ...
    {
        var client = factory.CreateHttpClient(identity, out var sharedSecret);
        var driveSvc = RefitCreator.RestServiceFor<IUniversalDriveV2HttpClientApi>(client, sharedSecret);
