...
namespace Odin.Hosting.Tests.AppAPI.Feed;

[TestFixture]
public class App_DataSubscriptionAndDistributionTests2
{
    ...
    public async Task EncryptedFile_UploadedByAnApp_IsOnlyDistributedTo_ConnectedFollowers_WithAccessInFileAcl_Of_A_SingleCircle()
    {
        ...
        await frodoOwnerClient.Transit.WaitForEmptyOutbox(firstUploadResult.File.TargetDrive);
    ...
    [Test]
    public async Task
        EncryptedFile_UploadedByAnApp_IsOnlyDistributedTo_ConnectedFollowers_WithAccessInFileAcl_Of_A_SingleCircle_And_Deleted_When_Owner_Deletes_File()
    {
        var frodoOwnerClient = _scaffold.CreateOwnerApiClient(TestIdentities.Frodo);
        var samOwnerClient = _scaffold.CreateOwnerApiClient(TestIdentities.Samwise);
        var merryOwnerClient = _scaffold.CreateOwnerApiClient(TestIdentities.Merry);
